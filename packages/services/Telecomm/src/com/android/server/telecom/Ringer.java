...
//caiji add for Care_OS incoming call speech start
import com.iflytek.business.speech.SynthesizerListener;
import com.android.magcomm.speech.SpeechTools;
import android.os.RemoteException;
//caiji add for Care_OS incoming call speech end
...
final class Ringer extends CallsManagerListenerBase {
    ...
    //caiji add for Care_OS incoming call speech start
    private static final String I99_TAG = "nian";
    private String name;
    private String coming;
    private String speech;
    SpeechTools mSpeechTools;
    //caiji add for Care_OS incoming call speech end
    ...
    Ringer(
		...
		//caiji add for Care_OS incoming call speech start
        mSpeechTools = new SpeechTools(context,AudioManager.STREAM_RING);
		//caiji add for Care_OS incoming call speech end
    }
    ...
    //caiji modify for synchronize ringer and UI
    //private void startRingingOrCallWaiting(Call call) {
    public void startRingingOrCallWaiting(Call call) {
...
//caiji add for Care_OS incoming call speech start
		...
		if (ringingCall != null){
            ...
            coming = mContext.getResources().getString(R.string.care_speech_incoming);
            speech = String.format(coming, name);
		}

        if (speech != null && speech.startsWith("+")){
            Log.i("caiji", "startRingingOrCallWaiting");
            speech = speech.substring(1);
        }
        Log.v(this, "startRingingOrCallWaiting ,jeff speech="+speech);
		if (mSpeechTools != null){
	        mSpeechTools.setSpeechContent(speech);
		}
	 	//caiji add for Care_OS incoming call speech end
        ...
        //@}
			...
			//caiji modify to still play with STREAM_RING when volume is 0	
            if (audioManager.getStreamVolume(AudioManager.STREAM_RING) >= 0) {
                ...
                mCallAudioManager.setIsRinging(call, true);
				//caiji modify for Care_OS incoming call speech start
                if(TelecomUtils.getRingingSpeechState(mContext) && mSpeechTools != null /*&& mSpeechTools.checkApplication()*/){//dengying@20160613
				 if(!mSpeechTools.isSpeaking()){
					 Log.v(this, "startRingingOrCallWaiting ,jeff = mSpeechTools != null");
//                     mRingtonePlayer.play(foregroundCall.getRingtone());
					 	mSpeechTools.startSpeech();
                    }
                }else{
                	Log.v(this, "startRingingOrCallWaiting ,jeff = mSpeechTools == null");
                if (SystemProperties.get("ro.mtk_multisim_ringtone").equals("1")) {
                    ...
                    Log.v(this, "startRingingOrCallWaiting ,jeff phoneAccountHandle= "+phoneAccountHandle);
                	...
                	Log.v(this, "startRingingOrCallWaiting ,jeff ro.mtk_multisim_ringtone");
		   	    ...
		   	    //caiji modify for Care_OS incoming call speech end
            } else {
...
//caiji modify for Care_OS incoming call speech start
		//caiji fix bug for still speech when other side hang up
        if(mSpeechTools != null){
            if(mSpeechTools.isSpeaking()){

package com.android.magcomm.speech;
...
public class SpeechTools{
    ...
    //dengying@20160613 begin
    public boolean checkApplication() {
		
		String packageName ="com.iflytek.speechsuite";
		
		Log.i(TAG,"checkApplication()");
       ...
       try {
			...
			Log.i(TAG,"NameNotFoundException e: " + e.getMessage());
			return false;
        }		
		
		/*try {
			ApplicationInfo info = mContext.getPackageManager().getApplicationInfo(packageName, PackageManager.GET_UNINSTALLED_PACKAGES);// GET_UNINSTALLED_PACKAGES代表已删除，但还有安装目录的
			return true;
		} catch (NameNotFoundException e) {
			Log.i(TAG,"NameNotFoundException e: " + e.getMessage());
			return false;

...
import com.magcomm.soundrecorder.widget.VUMeter;
...
public class SoundRecorder extends Activity implements SoundRecorderService.OnEventListener,
    ...
    private static final int OPTIONMENU_SELECT_OUT_FORMAT = 3;//hejianfeng add 
    ...
    private static final int DIALOG_SELECT_OUT_FORMAT = 3;//hejianfeng add 
    ...
    private static final String DIALOG_TAG_SELECT_OUT_FORMAT = "SelectOutFormat";//hejianfeng add 
    ...
    private int mSelectedOutFormat = -1;//hejianfeng add 
    ...
    private SharedPreferences mOutPrefs = null;//hejianfeng add 
	...
	private Switch mSwitchQuality;
	// vivo style add by y.haiyang (end)
	//hejianfeng add start 
	private boolean openOutFormat=true;
	//hejianfeng add end 
...
//hejianfeng add start
	 private void putIntEncoder(Editor editor,int encoder){
		editor.putInt(RecordParamsSetting.HIGH_ENCODER,encoder);
		editor.putInt(RecordParamsSetting.STANDARD_ENCODER,encoder);
		editor.putInt(RecordParamsSetting.LOW_ENCODER,encoder);
	 }
	 private DialogInterface.OnClickListener mSelectOutFormatListener = new DialogInterface.OnClickListener() {
        @Override
        public void onClick(DialogInterface dialog, int whichItemSelect) {
            LogUtils.i(TAG, "<mSelectOutFormatListener onClick>");
            mSelectedOutFormat=whichItemSelect;
            Editor editor = mOutPrefs.edit();
            int format = RecordParamsSetting.getOutSelectFormat(whichItemSelect);
            editor.putInt(RecordParamsSetting.HIGH_OUTPUT_FORMAT,format);
            editor.putInt(RecordParamsSetting.STANDARD_OUTPUT_FORMAT,format);
            editor.putInt(RecordParamsSetting.LOW_OUTPUT_FORMAT, format);
            switch (format) {
		        case MediaRecorder.OutputFormat.AMR_NB:
		            putIntEncoder(editor,MediaRecorder.AudioEncoder.AMR_NB);
		            break;
		        case MediaRecorder.OutputFormat.AMR_WB:
		            putIntEncoder(editor,MediaRecorder.AudioEncoder.AMR_WB);
		            break;
		        case MediaRecorder.OutputFormat.THREE_GPP:
		            putIntEncoder(editor,MediaRecorder.AudioEncoder.AAC);
		            break;
		        default:
		            break;
        	}
...
//              return;
    ...
    public void onClick(View button) {
		...
		case R.id.cancelButton:
			...
			mVUMeter.setStatus(VUMeter.POWER_OFF);
			mVUMeter.invalidate();
			onClickDiscardButton();
			break;
		// vivo style add by y.haiyang (end)
...
//    public void onCreate(Bundle icycle) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        //hejianfeng add start 
        else if(R.id.format_file == id){
			showDialogFragment(DIALOG_SELECT_OUT_FORMAT, null);
        }
        //hejianfeng add end 
        return true;
    }
...
//            return super.onKeyDown(keyCode, event);
    ...
    private void initResourceRefs() {
		...
		mCancelButton.setOnClickListener(this);// vivo style 
		...
		// vivo style add by y.haiyang (start)
		mSwitchQuality.setOnCheckedChangeListener(new MagCheckedListener());
		// vivo style add by y.haiyang (end)
    }
    ...
    private void showDialogFragment(int id, Bundle args) {
        ...
        case DIALOG_SELECT_EFFECT:
            ...
            break;
         //hejianfeng add start 
          case DIALOG_SELECT_OUT_FORMAT:
            removeOldFragmentByTag(DIALOG_TAG_SELECT_OUT_FORMAT);
            newFragment = SelectDialogFragment.newInstance(RecordParamsSetting
                    .getOutFormatStringIDArray(), null,
                    R.string.select_out_format_file, true, mSelectedOutFormat, null);
            ((SelectDialogFragment) newFragment).setOnClickListener(mSelectOutFormatListener);
            newFragment.show(fragmentManager, DIALOG_TAG_SELECT_OUT_FORMAT);
            LogUtils.i(TAG, "<showDialogFragment> show select out format dialog");
            break;
         //hejianfeng add end 
        default:
            break;
    ...
    public void setSelectedFormat(int which) {
	 ...
	 //hejianfeng add 
     // vivo style add by y.haiyang (start)
 		if(mSelectedFormat == RecordParamsSetting.FORMAT_HIGH ){
 			mSwitchQuality.setOnCheckedChangeListener(null);
 			mSwitchQuality.setChecked(true);
 			mSwitchQuality.setOnCheckedChangeListener(new MagCheckedListener());
 		}else if (mSelectedFormat == RecordParamsSetting.FORMAT_STANDARD){
 			mSwitchQuality.setChecked(false);
 		}
 	// vivo style add by y.haiyang (end)
    ...
    private void resetUi() {
		...
		// vivo style add by y.hayiang (start)
		if(VIVO){
			mRecordingStateImageView.setImageResource(R.drawable.light_gray);
		}else{
			mStopButton.setVisibility(View.GONE);
			mRecordingStateImageView.setVisibility(View.INVISIBLE);
		}
        // vivo style add by y.hayiang (eng)
        /**@}**/
    }
    ...
    private void initResourceRefsWhenNoService() {
		...
		mSwitchQuality = (Switch)findViewById(R.id.switch_quality);
		// vivo style add by y.haiyang (end)
    }
    ...
    private void updateUiOnIdleState() {
		...
		if(VIVO){
			mRecordingStateImageView.setImageResource(R.drawable.light_gray);
			mSwitchQuality.setEnabled(true);
		}else{
			mStopButton.setEnabled(false);
			mStopButton.setVisibility(View.GONE);
			mRecordingStateImageView.setVisibility(View.INVISIBLE);
        ...
        if (null == currentFilePath) {
            ...
            if (mRunFromLauncher) {
                ...
                mFileListButton.setEnabled(true);
                mFileListButton.setFocusable(true);
				mCancelButton.setVisibility(View.GONE);// vivo style
            } else {
                mFileListButton.setVisibility(View.GONE);
                mStopButton.setVisibility(View.VISIBLE);
                mStopButton.setEnabled(false);
            ...
            if (mRunFromLauncher) {
				...
				mPlayButton.setBackgroundResource(R.drawable.vivo_bn_play);
				// vivo style add by y.haiyang (end)
            }
            mRemainingTimeTextView.setVisibility(View.INVISIBLE);
    ...
    private void updateUiOnRecordingState() {
		...
		// vivo style add by y.hayiang (start)
		if(VIVO){
			mVUMeter.setStatus(VUMeter.POWER_ON);
			mCancelButton.setVisibility(View.VISIBLE);
            mCancelButton.setEnabled(true);
            mCancelButton.setFocusable(true);
			mSwitchQuality.setEnabled(false);
		}else{
			mVUMeter.setVisibility(View.VISIBLE);
		}
		// vivo style add by y.hayiang (eng)		
    ...
    private void updateUiOnPlayingState() {
		...
		mPlayButton.setBackgroundResource(R.drawable.vivo_bn_pause);
		// vivo style add by y.haiyang (end)
        mFileListButton.setVisibility(View.GONE);
        mPauseRecordingButton.setVisibility(View.GONE);
        mPauseRecordingButton.setSoundEffectsEnabled(false);
    ...
    private void restoreDialogFragment() {
        ...
        //hejianfeng add start 
        fragment = fragmentManager.findFragmentByTag(DIALOG_TAG_SELECT_OUT_FORMAT);
        if (null != fragment) {
            ((SelectDialogFragment) fragment).setOnClickListener(mSelectFormatListener);
        }
        //hejianfeng add end 
    ...
    private void restoreRecordParamsSettings() {
		...
		Log.i(TAG,"mSelectedFormat == " + mSelectedFormat);
		// vivo style add by y.haiyang (start)
		if(mSelectedFormat == RecordParamsSetting.FORMAT_HIGH ){
			mSwitchQuality.setOnCheckedChangeListener(null);
			mSwitchQuality.setChecked(true);
		...
		//hejianfeng add start
		if (null == mOutPrefs) {
            mOutPrefs = getSharedPreferences(RecordParamsSetting.RECORD_PARAM, 0);
        }
        int format=mOutPrefs.getInt(RecordParamsSetting.STANDARD_OUTPUT_FORMAT,
                    MediaRecorder.OutputFormat.THREE_GPP);
        for(int i=0;i<RecordParamsSetting.outFormatMax;i++){
			if(RecordParamsSetting.getOutSelectFormat(i)==format){
				mSelectedOutFormat=i;
			}
        }
		//hejianfeng add end 
    }
    ...
    private void updateOptionsMenu(boolean isShow) {
		...
		//hejianfeng add start
         if (openOutFormat) {
            MenuItem item4 = mMenu.getItem(OPTIONMENU_SELECT_OUT_FORMAT);
            if (null != item4) {
                item4.setVisible(allowSelectFormatAndMode);
            }
        }
		//hejianfeng add end
    }
...
//vivo style add by y.haiyang (start)
	private class MagCheckedListener implements Switch.OnCheckedChangeListener{
			
			@Override
			public void onCheckedChanged(CompoundButton view, boolean checked) {
				RecordParamsSetting.getFormatStringArray(SoundRecorder.this);
				Log.i(TAG,"checked = " + checked );
				int toastStr = -1;
				if(checked){
					setSelectedFormat(RecordParamsSetting.FORMAT_HIGH);
					toastStr = R.string.switch_tone_quality_higt;
				}else{
					setSelectedFormat(RecordParamsSetting.FORMAT_STANDARD);
					toastStr = R.string.switch_tone_quality_normal;
				}
				Toast.makeText(SoundRecorder.this, toastStr, Toast.LENGTH_SHORT).show();
			}
	}
	//vivo style add by y.haiyang (end)
	/**
     ...
     */
    private static final boolean isRunningInEmulator() {

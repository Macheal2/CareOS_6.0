...
import com.android.contacts.common.R;//wangcunxi modify 
...
public final class AccountsListAdapterUtils {
    ...
    public static void getViewForName(Context context, AccountWithDataSet account,
        ...
        android.util.Log.e("accountType","accountType:"+accountType);
        if(accountType != null){
            if (accountType.isIccCardAccount()) {
                //LogUtils.d("checkphoto", "accountlistadpter subId : " + subId);
                //icon.setImageDrawable(accountType.getDisplayIconBySubId(context, subId));
                text2.setText(context.getResources().getString(R.string.sim_contact));
                icon.setImageResource(R.drawable.i99_sim);
            } else if(AccountWithDataSetEx.isLocalPhone(accountType.accountType)){
                text2.setText(context.getResources().getString(R.string.local_contact));
                icon.setImageResource(R.drawable.i99_phone);
            }else{

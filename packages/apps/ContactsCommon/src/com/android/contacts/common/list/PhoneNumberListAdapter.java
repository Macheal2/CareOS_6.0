...
public class PhoneNumberListAdapter extends ContactEntryListAdapter {
    ...
    protected void bindPhoto(final ContactListItemView view, int partitionIndex, Cursor cursor) {
        ...
        //wangcunxi modify begin Care_os(bind photo is only one sim cared)
        final int subId = cursor.getInt(cursor.getColumnIndexOrThrow(Contacts.INDICATE_PHONE_SIM));
        if(subId > 0){
            view.getPhotoView().setImageResource(R.drawable.i99_ic_sim_action_call_bg);
        }else if (photoId != 0) {
            getPhotoLoader().loadThumbnail(view.getPhotoView(), photoId, false,
                    getCircularPhotos(), null);
        }else {
            view.getPhotoView().setImageResource(R.drawable.i99_ic_action_call_bg);
        }

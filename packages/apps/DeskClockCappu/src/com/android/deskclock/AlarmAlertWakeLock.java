...
public class AlarmAlertWakeLock {
    ...
    public static void acquireCpuWakeLock(Context context) {
        ...
        /// M:turn on the screen for smartBook {@
        turnSMBScreenOn(context);
        /// @}
    }
    ...
    private static void turnSMBScreenOn(Context context) {
        /// M:turn on the screen for smartBook {@
        DisplayManager mDisplayManager = (DisplayManager)
                context.getSystemService(Context.DISPLAY_SERVICE);
        if (mDisplayManager.isSmartBookPluggedIn()) {
            PowerManager pm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
            LogUtils.v("SmartBook is pluggedIn , turn on the screen of SmartBook");
            //pm.setSmartBookScreen(true);//modify by even
        }
        /// @}
    }
}

...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    public static int mSnoozeCount=0;//dengjianzhang@20151118
    ...
    /**
     * M: this method that can get the nearest alarm @{
     * @param context application context
     * @return AlarmInstance nextAlarm
     */
    ...
    public static void setMissedState(Context context, AlarmInstance instance) {
        ...
        /// @}
      //dengjianzhang@20151118 add start
        if(mSnoozeCount<5){
       	 	mSnoozeCount++;
    	 	Calendar currentTime = Calendar.getInstance();
    	 	Calendar timeoutTime = instance.getTimeout(context);
    	 	boolean hasTimeout = timeoutTime != null && currentTime.after(timeoutTime);
    	 	LogUtils.v("Alarm setMissedState hasTimeout = " + hasTimeout);
    	 	if (hasTimeout) {
    	 		LogUtils.v("Alarm timeout and set the alarm to snoozed state");
    	 		Intent AlarmIntent = new Intent();
    	 		AlarmIntent.setAction("com.android.deskclock.ALARM_SNOOZE");
    	 		context.sendBroadcast(AlarmIntent);
    	 		return;
    	 	}
        }
		mSnoozeCount=0;
	  //dengjianzhang@20151118 add end
	   // Stop alarm if this instance is firing it
	    AlarmService.stopAlarm(context, instance);
    ...
    private void handleIntent(Context context, Intent intent) {
        ...
        /// M: Update the notification, then launch the activity @{
            ...
            AlarmNotifications.updateAlarmNotification(context, instance);
            Intent i = AlarmInstance.createIntent(context, MagcommAlarmActivity.class, instance.mId);//dengjianzhang@20150519

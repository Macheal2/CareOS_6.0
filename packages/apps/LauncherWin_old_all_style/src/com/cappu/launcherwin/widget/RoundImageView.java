    ...
    private void setUpShader() {
        if (mImagePath == null) {
            if(getDrawable() == null){
                ...
                } else if(ThemeManager.getInstance().getCurrentThemeType(getContext()) == ThemeManager.THEME_SIMPLENESSSTYLE){
                    if(mViewType == ViewType.health){
                    	setBackgroundResource(R.drawable.application_travel_mode_6);// 汪洋 修改：之前新闻健康右边显示的是健康图片、这里替换成旅游
                        
                    }else if(mViewType == ViewType.news){
                        Log.i("WANGYANG", "news"+(mViewType == ViewType.news));
                        setBackgroundResource(R.drawable.application_news_mode_6);
                    }
        ...
        if(bmp == null){
            if(ThemeManager.getInstance().getCurrentThemeType(getContext()) == ThemeManager.THEME_CHINESESTYLE){ //added by wangyang
                ...
                if(mViewType == ViewType.finances){
                    bmp = BitmapFactory.decodeResource(getResources(), R.drawable.application_finances_mode_6_banner);
                }else if(mViewType == ViewType.travel){
                    bmp = BitmapFactory.decodeResource(getResources(), R.drawable.application_travel_mode_6_banner);
                }else if(mViewType == ViewType.health){
                    bmp = BitmapFactory.decodeResource(getResources(), R.drawable.application_health_mode_6_banner);
                }else if(mViewType == ViewType.news){
                    bmp = BitmapFactory.decodeResource(getResources(), R.drawable.application_news_mode_6_banner);
                }
            }else{
                if(mViewType == ViewType.finances){
...
//    }
    ...
    public void setImageShapeType(int type){
        ...
        if(ThemeManager.getInstance().getCurrentThemeType(getContext()) == ThemeManager.THEME_CLASSICAL || ThemeManager.getInstance().getCurrentThemeType(getContext()) == ThemeManager.THEME_CHINESESTYLE){

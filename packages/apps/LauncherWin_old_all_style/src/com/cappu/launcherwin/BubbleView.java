...
//		}
		...
		//START: added by Yar @20170824
		if (!isRegisterContacts && ic.mRawId != -1 && ic.phoneNumber != null && !ic.phoneNumber.equals("")) {
    ...
    private void setAppWidgetIcon(ItemInfo itemInfo,float density,int textViewHeight){
		...
		if(ThemeManager.getInstance().getCurrentThemeType() == ThemeManager.THEME_TWELVE_GRIDS
			...
			mRelativeLayoutHead.setPadding(10, 10, 10, 90);
	...
	class ContactsObserver extends ContentObserver {
		...
		public void onChange(boolean selfChange) {
			...
			} else if (isRegisterContacts && raw_id == -1) {
				mContext.getContentResolver().unregisterContentObserver(mObserver);
	        	isRegisterContacts = false;

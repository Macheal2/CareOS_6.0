...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    void setModeChangeCurrentPage(int cerrentPage){
        mCurrentPage = cerrentPage;
        mFirstLayout = true;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //MeasureSpec.EXACTLY是精确尺寸，当我们将控件的 layout_width或layout_height指定为具体数值时如
     ...
     * 
     * 第一种 
     *      如果当前viewgroup 的onInterceptTouchEvent 返回 true 将传递给自己的 onTouchEvent 方法
     *      如果onInterceptTouchEvent 返回 false 将传递给 子viewgroup的 onInterceptTouchEvent 方法
     * 第二种 
     *      如果 子viewgroup 的onInterceptTouchEvent 返回 true 将传递给 子viewgroup的 onTouchEvent 方法
     *      如果 子viewgroup 的onInterceptTouchEvent 返回 false 将传递给 子view的 onTouchEvent 方法
     *      
     *      如果 子viewgroup 的onTouchEvent 返回 false 那么事件将传递给父（就当前）的viewgroup 的 onTouchEvent 方法
     *      
     * */
    @Override
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        /// M: modify to cycle sliding screen.
        if (isSupportCycleSlidingScreen()) {
            if (getScrollX() <= 0 && mCurrentPage == 0 && whichPage == getChildCount() - 1) {
                mUnboundedScrollX = getChildCount() * getWidth() + getScrollX();
            } else if (getScrollX() >= mMaxScrollX && mCurrentPage == getChildCount() - 1 && whichPage == 0) {

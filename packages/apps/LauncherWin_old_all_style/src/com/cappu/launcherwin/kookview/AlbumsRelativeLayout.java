...
public class AlbumsRelativeLayout extends RelativeLayout implements OnChildViewClick {
    ...
    private class IShareImage extends ImageView {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if("abc".equals(mImagePath)){
                ...
                } else if(ThemeManager.getInstance().getCurrentThemeType(mContext) == ThemeManager.THEME_SIMPLENESSSTYLE){
                    bmp = BitmapFactory.decodeResource(getResources(), R.drawable.ishare_default_simpleness);
                } else{
  ...
  //人脸识别 add by wangyang 2016.10.21 start
    ...
    @Override
    public void run() {
        LauncherLog.v(TAG, "FaceThread,jeff mIShareFaceList="+mIShareFaceList+",mIShareDaoList="+mIShareDaoList);
        if(mIShareFaceList.size() == mIShareDaoList.size() && mIShareFaceList.containsAll(mIShareDaoList)){
            //两个集合相等
        } else {//否则不相等
            if(mIShareFaceList != null){
                mIShareFaceList.clear();
            ...
            if(mBitmapFactoryOptionsbfo == null){
                ...
                for(int a = 0; a<mIShareFaceList.size(); a++){
                    Bitmap mFaceBitmap = BitmapFactory.decodeFile(mIShareFaceList.get(a), mBitmapFactoryOptionsbfo);
                    Log.i(WANGFACE, "run  run"+"       mFaceBitmap = "+(mFaceBitmap == null));
                    if(mFaceBitmap != null){
                        mImageFaceWidth = mFaceBitmap.getWidth();
                        mImageFaceHeight = mFaceBitmap.getHeight();
                        myFace = new FaceDetector.Face[mMumberOfFace];
                        myFaceDetect = new FaceDetector(mImageFaceWidth, mImageFaceHeight, mMumberOfFace);
                        int getNumberFace = myFaceDetect.findFaces(mFaceBitmap, myFace);
                        if(getNumberFace != 0){
                            Face f  = myFace[0];
                            ...
                            if(f != null){
                                ...
                                Log.i(WANGFACE, "       width = "+width +"   height = "+height);
                                ...
                                if(faceWidth >= width && faceHeight >= height){
                                    ...
                                    if(eyeX <= width/2 && eyeY <= height /2){//1
                                        Log.i(WANGFACE, "   111111    ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, 0, 0, width, height);
                                    } else if(width/2 < eyeX && eyeX< (faceWidth -width/2) && eyeY< height/2){//2
                                        Log.i(WANGFACE, "    222222222   ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, eyeX - width/2, 0, width, height);
                                    } else if(eyeX >= (faceWidth -width/2) && eyeY <= height/2){//3
                                        Log.i(WANGFACE, "    333333   ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, faceWidth - width, 0, width, height);
                                    } else if(eyeX<= width/2 && eyeY >= height/2 && eyeY <= (faceHeight - height/2)){//4
                                        Log.i(WANGFACE, "    44444444   ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, 0, eyeY -height/2 , width, height);
                                    } else if(eyeX >= width/2 && eyeX <= (faceWidth - width/2) && eyeY >= height/2 && eyeY <= (faceHeight - height/2)){//5
                                        Log.i(WANGFACE, "      55555555 ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, eyeX -width/2, eyeY -height/2 , width, height);
                                    } else if(eyeX >= (faceWidth - width/2) && eyeY >= height/2 && eyeY <= (faceHeight - height/2)){//6
                                        Log.i(WANGFACE, "    6666666666   ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, faceWidth -width, eyeY -height/2 , width, height);
                                    } else if(eyeX <= width/2 && eyeY >= (faceHeight - height/2)){//7
                                        Log.i(WANGFACE, "    77777777   ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, 0, faceHeight - height, width, height);
                                    } else if(eyeX >= width/2 && eyeX < (faceWidth - width/2) && eyeY >= (faceHeight - height/2)){//8
                                        Log.i(WANGFACE, "     888888  ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, eyeX - width/2, faceHeight - height, width, height);
                                    } else if(eyeX >= (faceWidth - width/2) && eyeY >= (faceHeight - height/2)){//9
                                        Log.i(WANGFACE, "    9999   ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, faceWidth - width, faceHeight - height, width, height);
                                    } else{
                                        Log.i(WANGFACE, "    faceWidth >= width && faceHeight >= height   ");
                                    }
                                    ...
                                    if(eyeX <= width/2){//1
                                        Log.i(WANGFACE, "   11    ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, 0, 0, width, faceHeight);
                                    } else if(eyeX >= width/2 && eyeX <= (faceWidth - width/2)){//2
                                        Log.i(WANGFACE, "   12    ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, eyeX - width/2, 0, width, faceHeight);
                                    } else if(eyeX >= (faceWidth - width/2)){//3
                                        Log.i(WANGFACE, "    13   ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, eyeX - width, 0, width, faceHeight);
                                    } else{
                                        Log.i(WANGFACE, "    faceWidth >= width && faceHeight <= height   ");
                                    }
                                    ...
                                    if(eyeY <= height/2){//1
                                        Log.i(WANGFACE, "   14    ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, 0, 0, faceWidth, height);
                                    } else if(eyeY >= height/2 && eyeY <= (faceHeight - height/2)){//2
                                        Log.i(WANGFACE, "   15    ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, 0, eyeY - height/2, faceWidth, height);
                                    } else if(eyeY >= (faceHeight - height/2)){//3
                                        Log.i(WANGFACE, "    16   ");
                                        mFaceBitmap = Bitmap.createBitmap(mFaceBitmap, 0, faceHeight - height/2, faceWidth, height);
                                    } else{
                                        Log.i(WANGFACE, "    faceWidth <= width && faceHeight >= height   ");
                                    }
                                } else if(faceWidth <= width && faceHeight <= height){
                                ...
                                mFaceFileCache.setBitmapCache(mIShareFaceList.get(a), mFaceBitmap);
                            }
                        }
                    }
                }

...
public class FmService extends Service implements FmRecorder.OnRecorderStateChangedListener {
    ...
    private static final String ACTION_FM_POWERDOWN = "com.android.fmradio.FmService.ACTION_FM_POWERDOWN";
...
/* Cappu:lau on: Sat, 29 Jul 2017 13:54:42 +0800
 * TODO: modify AirPlaneMode on FMRadio off 
 */
            } else if (Intent.ACTION_AIRPLANE_MODE_CHANGED.equals(action)) {
                boolean mAirplaneModeOn = intent.getBooleanExtra("state", false) ? true : false;
                android.util.Log.i("liukun", "mAirplaneModeOn = " + mAirplaneModeOn);
                if (mAirplaneModeOn) {
                    mFmServiceHandler.removeCallbacksAndMessages(null);
                    exitFm();
                    stopSelf();
...
// End of Cappu:lau
            ...
            } else if (ACTION_FM_POWERDOWN.equals(action)) {// dengying@20150717
                                                            // fm status close
//                mActivityManager.forceStopPackage("com.android.fmradio");
                mFmServiceHandler.removeCallbacksAndMessages(null);
                exitFm();
                stopSelf();
            }
...
/* Cappu:lau on: Sat, 29 Jul 2017 13:56:42 +0800
 * TODO: modify AirPlaneMode on FMRadio off
 */
...
// End of Cappu:lau
    ...
    private boolean isPatchMixerToEarphone(ArrayList<AudioPatch> patches) {
        ...
        for (AudioPatch aPatch : patches) {
            ...
            if (sourcePort.id() <= minSourcePortId) {
                ...
                Log.d(TAG, "minSourcePortId =" + minSourcePortId);
            }
        }
    ...
    private void showPlayingNotification() {
        ...
        notificationIntent.setClassName(getPackageName(), mTargetClassName);
        PendingIntent pendingIntent = PendingIntent.getActivity(mContext, 0, notificationIntent, 0);
        Notification notification = new Notification(R.drawable.fm_title_icon, null,
                System.currentTimeMillis());
        notification.flags |= Notification.FLAG_ONGOING_EVENT | Notification.FLAG_NO_CLEAR;

        RemoteViews contentView = new RemoteViews(getPackageName(), R.layout.fm_notification_view);
        String fmUnit = mContext.getString(R.string.fm_symbol);
        String text = FmUtils.formatStation(mCurrentStation) + " " + fmUnit;
        
        contentView.setImageViewResource(com.android.internal.R.id.icon, R.drawable.fm_title_icon);
        contentView.setInt(com.android.internal.R.id.icon, "setBackgroundResource", com.android.internal.R.drawable.notification_icon_legacy_bg);
        contentView.setTextViewText(R.id.txt_app_name, getResources().getString(R.string.app_name));
        contentView.setTextViewText(R.id.txt_channel, text);
        
        notification.contentView = contentView;
        notification.contentIntent = pendingIntent;

        Intent fmPowerOffIntent = new Intent();
        fmPowerOffIntent.setAction(ACTION_FM_POWERDOWN);
        PendingIntent fmPowerOffPIntent = PendingIntent.getBroadcast(this, 0, fmPowerOffIntent, 0);
        contentView.setOnClickPendingIntent(R.id.layout_close, fmPowerOffPIntent);

...
/// @}
    ...
    private static final float SCALE_LIMIT = 1.3f;//0.703125f;	//the same doubletab scale second time modify by Yar @20170726
    ...
    public void scrollPage(int dx, int dy) {
        ...
        //START: modified by Yar @20170803
        if (b.mCurrentScale > SCALE_LIMIT) {
        	startAnimation(x, y, 1.0f, ANIM_KIND_SCROLL);
        } else {
        	startAnimation(x, y, b.mCurrentScale, ANIM_KIND_SCROLL);
        }
        //END: modified by Yar @20170803
    }
    ...
    */
    ...
    private float getMinimalScale(Box b) {
        ...
        /// @}
        //START: the same doubletab scale second time modify by Yar @20170726
        if (SCALE_LIMIT <= 2.0f) {
        	return s > 2.0f ? SCALE_LIMIT : s;//modified by Yar @20170803
        }
        //END: the same doubletab scale second time modify by Yar @20170726
    ...
    private class Platform extends Animatable {
        ...
        public boolean startSnapback() {
            ...
            //START: the same doubletab scale second time modify by Yar @20170727
            if (SCALE_LIMIT < 2.0f) {
            	//scaleMax = 2.0f;
            }
            //END: the same doubletab scale second time modify by Yar @20170727
    ...
    private class Box extends Animatable {
        ...
        public float clampScale(float s) {
        	//START: modify by Yar @20170727
        	/*
            return Utils.clamp(s,
                    SCALE_MIN_EXTRA * mScaleMin,
                    SCALE_MAX_EXTRA * mScaleMax);*/
        	
        	return Utils.clamp(s, mScaleMin, mScaleMax);
        	//END: modify by Yar @20170727
        }

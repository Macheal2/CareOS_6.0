...
/// @}
    ...
    private boolean mLeftScrollIntoGalleryEnd = false;//dengjianzhang@20151117 add
    ...
    private class MyGestureListener implements GestureRecognizer.Listener {
        ...
        public boolean onDoubleTap(float x, float y) {
            ...
            mIgnoreUpEvent = true;
            if (scale <= .70f || controller.isAtMinimalScale()) {//modify by even no more
				...
				controller.zoomIn(x, y, Math.min(scale * 1.5f, 1.3f));//modify by even Math.min(1.5f, Math.min(scale * 1.5f, 4.0f))
                /// @}
            } else {

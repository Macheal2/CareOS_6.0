...
public class CallDetailActivity extends Activity
    ...
    private I99OnClickListener mI99ClickListener;
    
    private Intent mTitleIntent;
    ...
    private CallLogAsyncTaskListener mCallLogAsyncTaskListener = new CallLogAsyncTaskListener() {
        ...
        public void onGetCallDetails(PhoneCallDetails[] details) {
            ...
            mName = TextUtils.isEmpty(firstDetails.name) ? null : firstDetails.name.toString();
            mType = firstDetails.numberType;
            ...
            findViewById(R.id.call_detail).setVisibility(View.VISIBLE);
			 ...
			 updateI99Header(firstDetails);
			 // yuan tong qin end
			 
			 
             //yuan tong qin add start 
             if (!isNew) {
                 mTitleIntent = new Intent(Intent.ACTION_VIEW, contactUri);
                 mTitleIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
             } else {
                 mTitleIntent = null;
              }   
        }
        ...
        // yuan tong qin add start 
        public  CharSequence forceLeftToRight(CharSequence text) {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        /// yuan tong qin start 
        mI99ClickListener = new I99OnClickListener();
        mTopBarListener = new TopBarListener();
        
        mTopBar = (TopBar)findViewById(R.id.topbar);
        mTopBar.setOnTopBarListener(mTopBarListener);
        
        //拨打电话和发短信
        callButton = (Button)findViewById(R.id.i99_action_call);
        callButton.setOnClickListener(mI99ClickListener);
        mMessageButton = (Button)findViewById(R.id.i99_action_message);
        mMessageButton.setOnClickListener(mI99ClickListener);
        // yuan tong qin end 
        ...
        //getActionBar().setDisplayHomeAsUpEnabled(true);//yuantongqin del
		getActionBar().hide();//yuantongqin add
...
//yuantongqin del
    ...
    private void updateI99Header(PhoneCallDetails details){

        if(isCustomerService(details)){
//            mTopBar.setText(getResources().getString(com.android.internal.R.string.customer_service_name));
            // yuan tong qin add 
        	 mTopBar.setText("null");
            mTopBar.setSubText(details.number.toString());
            return ;
        }

        if(TextUtils.isEmpty(details.name)){
            mTopBar.setText(details.number.toString());
            mTopBar.setSubText(R.string.i99_save_contact);
        }else{
            mTopBar.setText(details.name.toString());
            mTopBar.setSubText(details.number.toString());
        }
    }
    ...
    private void showOptionDialog(){
        if(mOptionDialog == null){
            mOptionDialog = new OptionDialog(CallDetailActivity.this);
            mOptionDialog.setTitle(R.string.i99_option);
            mOptionDialog.addButton(R.string.i99_ip_calls);
            mOptionDialog.addButton(R.string.i99_save_contact);
            mOptionDialog.addButton(R.string.i99_clear_contact_call_log);
            mOptionDialog.setOnClickListener(new OptionDialog.OnClickListener(){
                @Override
                public void onClick(View v){
                    switch(v.getId()){
                        case R.string.i99_ip_calls:
                        		doIPCall();
                            break;
                        case R.string.i99_save_contact:

                            break;
                        case R.string.i99_clear_contact_call_log:
                             CareDialog confirmDialog = new CareDialog.Builder(CallDetailActivity.this)
                                .setTitle(R.string.i99_warning)
                                .setMessage(R.string.i99_confirm_clear_calllog)
        ...
        Button ipCall = mOptionDialog.getItemById(R.string.i99_ip_calls);
        Button contact = mOptionDialog.getItemById(R.string.i99_save_contact);
    ...
    private boolean isCustomerService(PhoneCallDetails details){

        if(!I99Utils.isCustomerService(CallDetailActivity.this)){
            return false;
        }

        final String number = getResources().getString(R.string.customer_service_number);
//        final String number = "is null";
        if(!TextUtils.isEmpty(number) && number.equals(details.number)){
            return true;
        }else{
            return false;
        }
	...
	private class TopBarListener implements onTopBarListener {
		...
		public void onLeftClick(View v) {
			...
			intent.setClass(CallDetailActivity.this, com.android.dialer.calllog.CallLogActivity.class);
		...
		public void onTitleClick(View v) {
			...
			//if (isCustomerService(mDetails)) {
			//	Intent intent = new Intent();
			//	intent.setClassName("com.android.contacts", "com.cappu.contacts.I99CustomerServiceActivity");
			//}

			// yuan tong qin add start
			if (mTitleIntent != null){
				startActivity(mTitleIntent);
			}else{
				doCreateNew();
			}
		}
	...
	private void doSMS() {
		IntentProvider intentProvider = IntentProvider.getSendSmsIntentProvider(mNumber);
	...
	private void doIPCall() {

		IntentProvider intentProvider = IntentProvider.getIpDialCallIntentProvider(mNumber);
		if (intentProvider != null) {
			final Intent intent = intentProvider.getIntent(mContext);
			if (intent != null) {
				DialerUtils.startActivityWithErrorToast(mContext, intent);

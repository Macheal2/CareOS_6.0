...
public class PhoneCallDetailsHelper {
    ...
    public void setPhoneCallDetails(PhoneCallDetailsViews views, PhoneCallDetails details) {
        ...
        //设置宽度  yuan tong qin add begin
        int width=mContext.getResources().getDisplayMetrics().widthPixels;
        views.namecount.setLayoutParams(new LayoutParams(2*width/3, LayoutParams.MATCH_PARENT));
        ...
        //结束 yuantongqin add begin
        Drawable drawableone=mContext.getResources().getDrawable(R.drawable.care_item_bottom_end_bg);
        Drawable drawabletwo=mContext.getResources().getDrawable(R.drawable.care_item_bottom_bg);
        if(details.isBg){
        	views.callInfoBg.setBackground(drawableone);
        }else{
        	views.callInfoBg.setBackground(drawabletwo);
        }
        //yuantongqin add end
    ...
    public CharSequence getCallTypeOrLocation(PhoneCallDetails details) {
        ...
        if (!TextUtils.isEmpty(details.number)
            ...
            if (TextUtils.isEmpty(details.name) && !TextUtils.isEmpty(details.geocode)) {
                ...
                /// M: for plug-in, and use the ExtensionManager managed plug-in @{
                ICallerInfoExt callerInfoExt = ExtensionManager.getInstance()
                        .getCallerInfoExtension();
                if (callerInfoExt != null) {
                    /// M: Get the subId from the cache
                    int subId = mTelecomCallLogCache.getSubId(details.accountHandle);
                    numberFormattedLabel = callerInfoExt.getTypeLabel(mContext,
                            details.numberType, details.numberLabel, null, subId);
                } else {

...
import android.util.Log;//yuan tong qin add 
...
public class CallLogGroupBuilder {
    ...
    //yuan tong qin add 没有下一个
    public static final int DAY_GROUP_NONENEXT = -2;
    ...
    public void addGroups(Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            ...
            if (DialerFeatureOptions.isVolteConfCallLogSupport()) {
                currentConfCallId = cursor
                        .getLong(CallLogQuery.CONFERENCE_CALL_ID);
            }
            boolean sameConference = firstConfCallId == currentConfCallId
                    && currentConfCallId > 0;
            if (sameConference) {
                shouldGroup = true;
            } else if (firstConfCallId > 0 || currentConfCallId > 0) {
                shouldGroup = false;
            } else
            /// @}

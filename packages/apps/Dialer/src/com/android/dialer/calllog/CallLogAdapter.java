...
public class CallLogAdapter extends GroupingListAdapter
    ...
    protected void bindCallLogListViewHolder(ViewHolder viewHolder, int position) {
        ...
        if (PhoneNumberUtil.canPlaceCallsTo(number, numberPresentation)
                ...
                info = DialerFeatureOptions.CALL_LOG_UNION_QUERY ? cachedContactInfo
                        : mContactInfoCache.getValue(number, countryIso, cachedContactInfo);
        }
		...
		boolean isbg = false;// yuan tong qin add start
		...
		int nextGroup = getnextDayGroup(c, number, currentGroup);
		...
		if (currentGroup != previousGroup) {
			...
			// yuan tong qin add start
			views.dayGroupHeader.setText(getGroupDescription(currentGroup, date));
		} else {
			views.dayGroupHeader.setVisibility(View.GONE);
		}

		final PhoneCallDetails details = new PhoneCallDetails(mContext, number, numberPresentation, formattedNumber, isVoicemailNumber, isbg);
        ...
        if (confCallId > 0 && !mIsConfCallMemberList) {
            ...
            details.name = getConferenceCallName(c, count);
            ...
            Log.d(TAG, "Volte ConfCall numbers= " + numbers + ", date="
                    + details.date + ", name=" + details.name);
        } else {
		...
		views.showActions(mCurrentlyExpandedPosition == position);//dengying
...
// / @}
		...
		// View view = inflater.inflate(R.layout.call_log_list_item, parent,
		// false);

        views.bindimage(views.callType);
	}

	// yuan tong qin add
	public int getnextDayGroup(Cursor cursor, String numbera, int tian) {
		int positiona = cursor.getPosition();
		int dayGroupa = CallLogGroupBuilder.DAY_GROUP_NONE;
		int a = -1;
		while (cursor.moveToNext()) {
			long previousRowIda = cursor.getLong(CallLogQuery.ID);
			String number = cursor.getString(CallLogQuery.NUMBER);
			dayGroupa = getDayGroupForCall(previousRowIda);
			if (!number.equals(numbera) || tian != dayGroupa) {
				a = dayGroupa;
				break;
			}
		}
		cursor.moveToPosition(positiona);// 让游标又返回到现在的位置
		return a;
	}
     ...
     */
	private CharSequence getGroupDescription(int group, long millistime) {
       if (group == CallLogGroupBuilder.DAY_GROUP_TODAY) {
           ...
           return mContext.getResources().getString(R.string.call_log_header_yesterday);
			...
			String milli = sdf.format(date);
			if (milli != null && !milli.equals("")) {
				return milli;
			} else {
    ...
    private String getConferenceCallName(Cursor cursor, int count) {
        ...
        for (int index = 0; index < count; ++index) {
            ///M: [Union Query] if Support Union Query, get the name from cursor
            String name = DialerFeatureOptions.CALL_LOG_UNION_QUERY ? cursor
                    .getString(CallLogQuery.CALLS_JOIN_DATA_VIEW_DISPLAY_NAME)
                    : getName(cursor);

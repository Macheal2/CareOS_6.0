...
public class LunarCalendar {
    ...
    private String [] mLunarDate0 , mLunarDate1 ;
    private String mLunar;
    private String mMonth;
    private String mLeapMonth ;
    private String mLunarDate10 ,mLunarDate20 ,mLunarDate30;
    public LunarCalendar(Context context){
        mResources = context.getResources();
       
        mLunarMonth = mResources.getStringArray(R.array.i99_lunar_month);
        mLunarDate0 = mResources.getStringArray(R.array.i99_date_0);
        mLunarDate1 = mResources.getStringArray(R.array.i99_date_1);
        
        mMonth = mResources.getString(R.string.i99_month);
        mLunar = mResources.getString(R.string.lunar);
        mLeapMonth = mResources.getString(R.string.i99_leap);
        mLunarDate10 = mResources.getString(R.string.i99_date_10);
        mLunarDate20 = mResources.getString(R.string.i99_date_20);
        mLunarDate30 = mResources.getString(R.string.i99_date_30);
    }
    ...
    public static final int[] solarToLunar(int year, int month, int monthDay) {
        ...
        for (iMonth = 1; iMonth <= 13 && offset > 0; iMonth++) {
            ...
            Log.i("HHJ", "iMonth:"+iMonth+"     daysOfMonth:"+daysOfMonth+"    offset:"+offset);
            offset -= daysOfMonth;
        }
        
        Log.i("HHJ", "最后是  iMonth:"+iMonth+"     daysOfMonth:"+daysOfMonth+"    offset:"+offset);
        
        
        // 当前月超过闰月，要校正
        if (leapMonth != 0 && iMonth > leapMonth) {
            
            --iMonth;//这里有个减减是因为上面的循环最后有一个 ++ 果然这里来一个减减 得到的是当前真正的月份
            

            if ((iMonth-1) == leapMonth) {//真正的月份减一 等于闰月得到的是闰月的月份
                isLeap = true;
                
                if(offset == 0){    //offset == 0  是防止初一的时候显示不对
                    isLeap = false;
                }
                
            }else if(iMonth == leapMonth){//这里当前月等于 闰月月份说明是正常月份 非闰月情况
              //offset为0时，并且刚才计算的月份是闰月，要校正
                if (offset == 0 && iMonth == leapMonth && daysInLunarMonth(iYear, iMonth) == daysOfMonth) { //offset == 0  是防止初一的时候显示不对
                    isLeap = true;
                    iMonth = iMonth;
                }else{
                    iMonth = iMonth+1;
        ...
        if (offset < 0) {
            offset += daysOfMonth;
            /*if(!isLeap){//如果是闰月就不需要校正了，前面校正了
                --iMonth;//这里要 减减 是因为在上面循环的时候 最后 加加了 一次
            }*/
            --iMonth;
        }
    ...
    public String getLunar(int year, int month, int day) {
        ...
        String s;
        s = mLunar ;
        s += (getIsLeap(lunar[3]) ? mLeapMonth : "") + mLunarMonth[lunar[1]-1] + mMonth ;
        s += cDay(lunar[2]) + " ";
        
        return s;
    }
    
    private  boolean getIsLeap(int leap) {
        if(leap == 1){
            return true;
        }else{
            return false;

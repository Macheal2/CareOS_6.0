...
public class CallerInfo {
    ...
    /* package */ CallerInfo markAsEmergency(Context context) {
        ...
        //caiji modify emergency call photo start
        //photoResource = R.drawable.img_phone;
        photoResource = R.drawable.img_emergency;
        //caiji modify emergency call photo end
    ...
    */
    ...
    /// @}
        ...
        /// M: [log optimize] add log for debugging potential time-consuming flow.
        checkTimingForDebugging(startTime, "get PhoneNumberUtil instance");
        ...
        */
        ...
        /// M: [log optimize] add log for debugging potential time-consuming flow.
        checkTimingForDebugging(startTime, "parse PhoneNumber");

        if (pn != null) {
            startTime = SystemClock.currentThreadTimeMillis();
            String description = geocoder.getDescriptionForNumber(pn, locale);
            /// M: [log optimize] add log for debugging potential time-consuming flow.
            checkTimingForDebugging(startTime, "get description for number");
    ...
    /**
     * M: [log optimize]Debugging utils to check potential time-consuming flow.
     * @param startTime the flow begin time. get by SystemClock.currentThreadTimeMillis().
     * @param description the description in log.
     */
    private static void checkTimingForDebugging(long startTime, String description) {
        long diffTime = SystemClock.currentThreadTimeMillis() - startTime;
        if (diffTime > 1000) {
            Log.w(TAG, "It spend too long to " + description + ": " + diffTime);
        }
    }
}

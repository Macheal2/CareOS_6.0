...
import android.provider.ContactsContract.CommonDataKinds.Photo;//yuan tong qin add 
        ...
        if (updatedPhotos != null) {
            ...
            for (String key : updatedPhotos.keySet()) {
                ...
                if(key.equals("photo_key")){
                    photoName = updatedPhotos.getString(key);
                    continue;
                }
                //wangcunxi add end
                ...
                values.put("photo_key", photoName);
                android.util.Log.e("wangcunxi","photoName::"+values.get("photo_key")+"___rawContactId___"+rawContactId);
                Cursor dataCursor = resolver.query(Data.CONTENT_URI,null,"raw_contact_id=?", new String[] { rawContactId + "" }, null);
                while (dataCursor.moveToNext()) {
                    String mime = dataCursor.getString(dataCursor.getColumnIndex(Data.MIMETYPE));
                    if(Photo.CONTENT_ITEM_TYPE.equals(mime)){
                        int data_id = dataCursor.getInt(dataCursor.getColumnIndex("_id"));
                        long rowsUpdated = resolver.update(ContentUris.withAppendedId(Data.CONTENT_URI, data_id),values, null, null);
                        android.util.Log.e("wangcunxi","rowsUpdated:"+rowsUpdated);
                    }
                }
                dataCursor.close();
                //wangcunxi add end

...
public class BatteryHistoryChart extends View {
    ...
    void finishPaths(int w, int h, int levelh, int startX, int y, Path curLevelPath,
        ...
        if (lastCharging) {
            mChargingPath.lineTo(w, h-mChargingOffset+6);
        }
        if (lastScreenOn) {
            mScreenOnPath.lineTo(w, h-mScreenOnOffset+6);
        }
        if (lastGpsOn) {
            mGpsOnPath.lineTo(w, h-mGpsOnOffset+6);
        }
        if (lastFlashlightOn) {
            mFlashlightOnPath.lineTo(w, h-mFlashlightOnOffset+6);
        }
        if (lastCameraOn) {
            mCameraOnPath.lineTo(w, h-mCameraOnOffset+6);
        }
        if (lastWifiRunning) {
            mWifiRunningPath.lineTo(w, h-mWifiRunningOffset+6);
        }
        if (lastCpuRunning) {
            mCpuRunningPath.lineTo(w, h - mCpuRunningOffset+6);
        }
        if (mHavePhoneSignal) {
                ...
                if (rec.isDeltaData()) {
                    ...
                    if (mLargeMode) {
                        ...
                        if (charging != lastCharging) {
                            if (charging) {
                                mChargingPath.moveTo(x, h-mChargingOffset+6);
                            } else {
                                mChargingPath.lineTo(x, h-mChargingOffset+6);
                            }
                        ...
                        if (screenOn != lastScreenOn) {
                            if (screenOn) {
                                mScreenOnPath.moveTo(x, h-mScreenOnOffset+6);
                            } else {
                                mScreenOnPath.lineTo(x, h-mScreenOnOffset+6);
                            }
                        ...
                        if (gpsOn != lastGpsOn) {
                            if (gpsOn) {
                                mGpsOnPath.moveTo(x, h-mGpsOnOffset+6);
                            } else {
                                mGpsOnPath.lineTo(x, h-mGpsOnOffset+6);
                            }
                            lastGpsOn = gpsOn;
                        }
                        ...
                        if (flashlightOn != lastFlashlightOn) {
                            if (flashlightOn) {
                                mFlashlightOnPath.moveTo(x, h-mFlashlightOnOffset+6);
                            } else {
                                mFlashlightOnPath.lineTo(x, h-mFlashlightOnOffset+6);
                            }
                        ...
                        if (cameraOn != lastCameraOn) {
                            if (cameraOn) {
                                mCameraOnPath.moveTo(x, h-mCameraOnOffset+6);
                            } else {
                                mCameraOnPath.lineTo(x, h-mCameraOnOffset+6);
                            }
                        ...
                        if (wifiRunning != lastWifiRunning) {
                            if (wifiRunning) {
                                mWifiRunningPath.moveTo(x, h-mWifiRunningOffset+6);
                            } else {
                                mWifiRunningPath.lineTo(x, h-mWifiRunningOffset+6);
                            }
                        ...
                        if (cpuRunning != lastCpuRunning) {
                            if (cpuRunning) {
                                mCpuRunningPath.moveTo(x, h - mCpuRunningOffset+6);
                            } else {
                                mCpuRunningPath.lineTo(x, h - mCpuRunningOffset+6);
                            }
    ...
    void drawChart(Canvas canvas, int width, int height) {
        ...
        if (mHavePhoneSignal) {
            ...
            int top = height-mPhoneSignalOffset+6 - (mLineWidth/2);

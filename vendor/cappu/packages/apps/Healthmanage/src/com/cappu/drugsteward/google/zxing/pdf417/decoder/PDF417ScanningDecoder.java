...
package com.cappu.drugsteward.google.zxing.pdf417.decoder;

import com.cappu.drugsteward.google.zxing.ChecksumException;
...
import com.cappu.drugsteward.google.zxing.common.BitMatrix;
import com.cappu.drugsteward.google.zxing.common.DecoderResult;
import com.cappu.drugsteward.google.zxing.pdf417.PDF417Common;
import com.cappu.drugsteward.google.zxing.pdf417.decoder.ec.ErrorCorrection;
...
public final class PDF417ScanningDecoder {
    ...
    // TODO don't pass in minCodewordWidth and maxCodewordWidth, pass in barcode
    // columns for start and stop pattern
    // columns. That way width can be deducted from the pattern column.
    // This approach also allows to detect more details about the barcode, e.g.
    // if a bar type (white or black) is wider
    // than it should be. This can happen if the scanner used a bad blackpoint.
    public static DecoderResult decode(BitMatrix image, ResultPoint imageTopLeft, ResultPoint imageBottomLeft, ResultPoint imageTopRight,
            ResultPoint imageBottomRight, int minCodewordWidth, int maxCodewordWidth) throws NotFoundException, FormatException, ChecksumException {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (i == 0
                    && (detectionResult.getBoundingBox().getMinY() < boundingBox.getMinY() || detectionResult.getBoundingBox().getMaxY() > boundingBox
                            .getMaxY())) {
        ...
        for (int barcodeColumnCount = 1; barcodeColumnCount <= maxBarcodeColumn; barcodeColumnCount++) {
            ...
            if (detectionResult.getDetectionResultColumn(barcodeColumn) != null) {
                // This will be the case for the opposite row indicator column,
                // which doesn't need to be decoded again.
                continue;
            }
            ...
            // TODO start at a row for which we know the start position, then
            // detect upwards and downwards from there.
            for (int imageRow = boundingBox.getMinY(); imageRow <= boundingBox.getMaxY(); imageRow++) {
                ...
                Codeword codeword = detectCodeword(image, boundingBox.getMinX(), boundingBox.getMaxX(), leftToRight, startColumn, imageRow, minCodewordWidth,
                        maxCodewordWidth);
                if (codeword != null) {
    ...
    private static DetectionResultRowIndicatorColumn getRowIndicatorColumn(BitMatrix image, BoundingBox boundingBox, ResultPoint startPoint,
            boolean leftToRight, int minCodewordWidth, int maxCodewordWidth) {
    ...
    private static void adjustCodewordCount(DetectionResult detectionResult, BarcodeValue[][] barcodeMatrix) throws NotFoundException {
        ...
        if (numberOfCodewords.length == 0) {
            ...
            // The calculated one is more reliable as it is derived from the row
            // indicator columns
            barcodeMatrix[0][1].setValue(calculatedNumberOfCodewords);
        }
    }
    ...
    private static DecoderResult createDecoderResult(DetectionResult detectionResult) throws FormatException, ChecksumException, NotFoundException {
        ...
        return createDecoderResultFromAmbiguousValues(detectionResult.getBarcodeECLevel(), codewords, PDF417Common.toIntArray(erasures),
                PDF417Common.toIntArray(ambiguousIndexesList), ambiguousIndexValues);
    }

    /**
     * This method deals with the fact, that the decoding process doesn't always
     * yield a single most likely value. The current error correction
     * implementation doesn't deal with erasures very well, so it's better to
     * provide a value for these ambiguous codewords instead of treating it as
     * an erasure. The problem is that we don't know which of the ambiguous
     * values to choose. We try decode using the first value, and if that fails,
     * we use another of the ambiguous values and try to decode again. This
     * usually only happens on very hard to read and decode barcodes, so
     * decoding the normal barcodes is not affected by this.
     * 
     * @param ecLevel
     * @param codewords
     * @param erasureArray
     ...
     *            array with the indexes that have more than one most likely
     *            value
     * @param ambiguousIndexValues
     *            two dimensional array that contains the ambiguous values. The
     *            first dimension must be the same length as the
     *            ambiguousIndexes array
     * @return
     * @throws FormatException
     * @throws ChecksumException
     */
    private static DecoderResult createDecoderResultFromAmbiguousValues(int ecLevel, int[] codewords, int[] erasureArray, int[] ambiguousIndexes,
            int[][] ambiguousIndexValues) throws FormatException, ChecksumException {
    ...
    private static Codeword detectCodeword(BitMatrix image, int minColumn, int maxColumn, boolean leftToRight, int startColumn, int imageRow,
            int minCodewordWidth, int maxCodewordWidth) {
        startColumn = adjustCodewordStartColumn(image, minColumn, maxColumn, leftToRight, startColumn, imageRow);
        // we usually know fairly exact now how long a codeword is. We should
        // provide minimum and maximum expected length
        // and try to adjust the read pixels, e.g. remove single pixel errors or
        // try to cut off exceeding pixels.
        // min and maxCodewordWidth should not be used as they are calculated
        // for the whole barcode an can be inaccurate
        ...
        // use start (and maybe stop pattern) to determine if blackbars are
        // wider than white bars. If so, adjust.
        // should probably done only for codewords with a lot more than 17 bits.
        // The following fixes 10-1.png, which has wide black bars and small
        // white bars
        ...
        // We could also use the width of surrounding codewords for more
        // accurate results, but this seems
        // sufficient for now
        if (!checkCodewordSkew(codewordBitCount, minCodewordWidth, maxCodewordWidth)) {
            // We could try to use the startX and endX position of the codeword
            // in the same column in the previous row,
            // create the bit count from it and normalize it to 8. This would
            // help with single pixel errors.
            return null;
        }
    ...
    private static int adjustCodewordStartColumn(BitMatrix image, int minColumn, int maxColumn, boolean leftToRight, int codewordStartColumn, int imageRow) {
        ...
        // are, then we need to start earlier.
        for (int i = 0; i < 2; i++) {
     ...
     * errors, attempts to correct the errors in-place.
     * </p>
     * 
     * @param codewords
     ...
     *            number of error correction codewords that are available in
     *            codewords
     * @throws ChecksumException

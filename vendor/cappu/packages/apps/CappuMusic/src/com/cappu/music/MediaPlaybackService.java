...
import com.cappu.music.bluetooth.avrcp.IBTAvrcpMusic;
import com.cappu.music.bluetooth.avrcp.ServiceAvrcpStub;
...
public class MediaPlaybackService extends Service {
    ...
    public static final String CLOSE_MUSIC = "com.android.kook.closemusic";
    public static final String PLAY_MUSIC = "com.android.kook.playmusic";
    ...
    private int mRepeatMode = REPEAT_ALL;//改默认播放模式
    ...
    public static long mNowPlayList=-1;//当前播放的音乐属于的音乐列表
...
//        mRemoteControlClient.setTransportControlFlags(flags);
        ...
        commandFilter.addAction(CLOSE_MUSIC);
        commandFilter.addAction(PLAY_MUSIC);
    ...
    public IBinder onBind(Intent intent) {
        ...
        /*Log.i("hehangjun","-----------------------------------------------------------------------------------------------------------------------");
        Log.i("hehangjun", String.format("intent %s stubname:%s", intent.getAction(), ServiceAvrcpStub.class.getName()));
        Log.i("hehangjun", "   IBTAvrcpMusic.class.getName() :   "+IBTAvrcpMusic.class.getName());*/
        //MusicLogUtils.v("SERVICE_AVRCP_STUB", String.format("intent %s stubname:%s", intent.getAction(), ServiceAvrcpStub.class.getName()));
        if (IBTAvrcpMusic.class.getName().equals(intent.getAction())) {
            MusicLogUtils.d("SERVICE_AVRCP_STUB", "MediaPlayer returns IBTAvrcpMusic");
            return mBinderAvrcp;
        } else if ("com.cappu.music.IMediaPlaybackService".equals(intent.getAction())) {
            MusicLogUtils.d("SERVICE_AVRCP_STUB", "MediaPlayer returns ServiceAvrcp inetrface");
            return mBinderAvrcp;
        }
        /// BT AVRCP End @}
        ...
        if (intent != null && !isEventFromMonkey()) {
            ...
            /// @}
                ...
                if (shuffle == MediaPlaybackService.SHUFFLE_NONE) {
                    ...
                    if (getRepeatMode() == MediaPlaybackService.REPEAT_CURRENT) {
                        setRepeatMode(MediaPlaybackService.REPEAT_ALL);
                    }
                ...
                cancelNotificationManager.cancel(PLAYBACKSERVICE_STATUS);
                cancelNotificationManager.cancelAll();
            }else if(PLAY_MUSIC.equals(action)){
                play();
                updateNotification(MediaPlaybackService.this,mAlbumArt);
            }else if (CMDPAUSE.equals(cmd) || PAUSE_ACTION.equals(action)) {
                pause();
                ...
                try {
                    if(getAudioId() != -1){
                        bm = MusicUtils.getArtwork(MediaPlaybackService.this, getAudioId(), -1, true);
                    }
                    if (bm == null) {
                        bm = MusicUtils.getDefaultArtwork(MediaPlaybackService.this,-1);
                    }
                } catch (IllegalArgumentException ex) {
                }
                updateNotification(MediaPlaybackService.this,/*mAlbumArt*/bm);
                mPausedByTransientLossOfFocus = false;
            }
        }
    ...
    private void notifyChange(String what) {
        ...
        Log.i("hehangjun", "发送广播 notifyChange  what :"+what);
...
//            ed.apply();
    ...
    private void openCurrentAndNext() {
        synchronized (this) {
            ...
            mCursor = getCursorForId(mPlayList[mPlayPos]);
            while ((mCursor == null) || !open(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI + "/" + mCursor.getLong(IDCOLIDX))) {
    ...
    public boolean open(String path) {
        synchronized (this) {
            ...
            Log.i("hehangjun", "1665    path:(" + path + ")");
            mFileToPlay = path;
    ...
    public void play() {
        synchronized(this) {
           final long short_songe_length = 10000;
            ...
            //如果play()被调用，onPrepare（）应该调用播放（）与音乐不应该是沉默多，如果发生错误
            mQuietMode = false;
            ///还要检查player准备
            if (mPlayer.isInitialized() && mIsPlayerReady) {
                ///重置播放列表完成
            ...
            else if(mPlayListLen <= 0 )
            {
                ...
                String  mSortOrder = "title_pinyin_key";
                Cursor cursor=getContentResolver().query(MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, mCursorCols, mWhere.toString(), null, mSortOrder);
                
               long[] lista=MusicUtils.getSongListForCursor(cursor);
               mShuffleMode=SHUFFLE_NONE;
               if(mPlayPos<=0)
               {
                   open(lista, 0);
               ...
               Log.i("test","====play()=  SHUFFLE_AUTO==="+SHUFFLE_AUTO); 
    ...
    private void updateNotification(Context context, Bitmap bitmap) {
        ...
        RemoteViews viewsBig = new RemoteViews(getPackageName(), R.layout.newstatusbar_big);
        ...
        viewsBig.setTextViewText(R.id.txt_trackinfo, trackinfo);
        viewsBig.setTextViewText(R.id.txt_artist, artist);
        Intent intent;
        ...
        viewsBig.setOnClickPendingIntent(R.id.iv_cover, pIntent);
        ...
        viewsBig.setOnClickPendingIntent(R.id.btn_prev, pIntent);
        ...
        if(isPlaying()){
            ...
            viewsBig.setImageViewResource(R.id.btn_pause, R.drawable.stat_notif_pause);
            
            views.setOnClickPendingIntent(R.id.btn_pause, pIntent);
            viewsBig.setOnClickPendingIntent(R.id.btn_pause, pIntent);
        }else{
            intent = new Intent(PLAY_MUSIC);
            intent.setClass(context, MediaPlaybackService.class);
            pIntent = PendingIntent.getService(context, 0, intent, 0);
            
            views.setImageViewResource(R.id.btn_pause, R.drawable.stat_notif_play);
            viewsBig.setImageViewResource(R.id.btn_pause, R.drawable.stat_notif_play);
        ...
        viewsBig.setOnClickPendingIntent(R.id.btn_next, pIntent);
          
        intent = new Intent("my.nullaction");
        pIntent = PendingIntent.getService(context, 0, intent, 0);
        views.setOnClickPendingIntent(R.id.rl_newstatus, pIntent);
        viewsBig.setOnClickPendingIntent(R.id.rl_newstatus, pIntent);
        if (bitmap != null) {
            views.setImageViewBitmap(R.id.iv_cover, getRoundedCornerBitmap(bitmap));
            viewsBig.setImageViewBitmap(R.id.iv_cover, getRoundedCornerBitmap(bitmap));
            mAlbumArt = bitmap;
        }
        
        intent = new Intent(CLOSE_MUSIC);
        intent.setClass(context, MediaPlaybackService.class);
        pIntent = PendingIntent.getService(context, 0, intent, 0);
        viewsBig.setOnClickPendingIntent(R.id.close_music, pIntent);
        ...
        mNotification = mNotifyBuilder.setSmallIcon(R.drawable.stat_notify_musicplayer).build();

        mNotification.contentView = views;
        mNotification.bigContentView = viewsBig;
        mNotification.flags |= Notification.FLAG_ONGOING_EVENT; 
        mNotification.contentIntent = PendingIntent.getService(context, 0, intent, 0);
        NotificationManager mNotifyManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        mNotifyManager.notify(PLAYBACKSERVICE_STATUS, mNotification);
    ...
    public void prev() {
        synchronized (this) {
            ...
            else if(mShuffleMode == SHUFFLE_AUTO)
            {
                ...
                Log.i("test", "hhhhhhhhhhhhhhhhhhhh播放模式=="+mShuffleMode);
                if(mRepeatMode == REPEAT_NONE)
                {
                    Random rand=new Random();
                    if(mPlayListLen>0)
                    {
                        int randnum=rand.nextInt(mPlayListLen);
                        mPlayPos=randnum; 
                    }
                    else
                    {
...
//                    return -1;
    ...
    private class MultiPlayer {
        ...
        public void setNextDataSource(String path) {
            ...
            //MusicLogUtils.i(TAG, "setNextDataSource: database duration = " + nextPlayerDuration + ", data = " + data + ", isDrm = " + isDrm);
            cursor.close();
            cursor = null;
        ...
        MediaPlayer.OnInfoListener infoListener = new MediaPlayer.OnInfoListener() {
            public boolean onInfo(MediaPlayer mediaPlayer, int what, int msg) {
                ...
                switch (what) {
                    ...
                    case 804://public static final int MEDIA_INFO_SEEKABLE = 804;
                        if (mediaPlayer.equals(mCurrentMediaPlayer)) {
                            mMediaSeekable = true;
                        } else {
                            mNextMediaSeekable = true;
                    ...
                    case 862://public static final int MEDIA_INFO_AUDIO_NOT_SUPPORTED = 862;
        ...
        public void setNextMediaPlayer(MediaPlayer next) {
            if (mCompatMode) {
                ...
                try {
                    ...
                    Log.i("HHJ", "setNextMediaPlayer:"+e.toString());
                }
            }
        }
    ...
    private class AlbumArtWorker extends AsyncTask<Long, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(Long... albumId) {
            ...
            try {
                ...
                if(getAudioId() != -1){
                    bm = MusicUtils.getArtwork(MediaPlaybackService.this, getAudioId(), id, true);
                }else{
                ...
                if (bm == null) {
                    bm = MusicUtils.getDefaultArtwork(MediaPlaybackService.this,id);
                }
            } catch (IllegalArgumentException ex) {
        ...
        protected void onPostExecute(Bitmap bm) {
            ...
            updateAlbumArt(MediaPlaybackService.this,bm);
            MusicLogUtils.d(TAG, "<< AlbumArtWorker.onPostExecute");
        }
    }
    private void updateAlbumArt(Context context, Bitmap bitmap) {
        //mAppWidgetProvider.performUpdateAlbum(MediaPlaybackService.this,bitmap);
    }

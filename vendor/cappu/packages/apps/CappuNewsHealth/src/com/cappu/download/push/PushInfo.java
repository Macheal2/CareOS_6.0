...
public class PushInfo {
    ...
    public static class Reader {
        ...
        public PushInfo newDownloadInfo(Context context,Uri queryUri,SystemFacade systemFacade,PushTypeSharedPreferences pushTypeSharedPreferences) {
            PushInfo info = new PushInfo(context,queryUri,systemFacade,pushTypeSharedPreferences);
            updateFromDatabase(info);
            readRequestHeaders(info);
            return info;
        }

        public void updateFromDatabase(PushInfo info) {
            info.mId = getLong(PushSettings.BasePushColumns._ID);
            info.mOtherAttachedId =  getLong(PushSettings.BasePushColumns.COLUMN_ATTACHED_ID);
            info.mUri = getString(info.mUri, PushSettings.BasePushColumns.COLUMN_URI);
            //info.mNoIntegrity = getInt(Downloads.COLUMN_NO_INTEGRITY) == 1;
            info.mHint = getString(info.mHint, PushSettings.BasePushColumns.COLUMN_FILE_NAME_HINT);
            info.mFileName = getString(info.mFileName, PushSettings.BasePushColumns._DATA);
            info.mMimeType = getString(info.mMimeType, PushSettings.BasePushColumns.COLUMN_MIME_TYPE);
            info.mPushType = getInt(PushSettings.PushType.PUSH_TYPE);
            info.mPushNameCH = getString(info.mPushNameCH, PushSettings.PushType.PUSH_TYPE_NAME);
            //info.mDestination = getInt(Downloads.COLUMN_DESTINATION);
            //info.mVisibility = getInt(Downloads.COLUMN_VISIBILITY);
            info.mStatus = getInt(PushSettings.BasePushColumns.COLUMN_STATUS);
            info.mNumFailed = getInt(PushSettings.BasePushColumns.FAILED_CONNECTIONS);
            int retryRedirect = getInt(PushSettings.BasePushColumns.RETRY_AFTER_X_REDIRECT_COUNT);
            info.mRetryAfter = retryRedirect & 0xfffffff;
            info.mLastMod = getLong(PushSettings.COLUMN_LAST_MODIFICATION);
            ...
            info.mTotalBytes = getLong(PushSettings.BasePushColumns.COLUMN_TOTAL_BYTES);
            info.mCurrentBytes = getLong(PushSettings.BasePushColumns.COLUMN_CURRENT_BYTES);
            //info.mETag = getString(info.mETag, PushSettings.ETAG);
            info.mDeleted = getInt(PushSettings.BasePushColumns.COLUMN_DELETED) == 1;
            //info.mIsPublicApi = getInt(Downloads.COLUMN_IS_PUBLIC_API) != 0;
            info.mAllowedNetworkTypes = getInt(PushSettings.COLUMN_ALLOWED_NETWORK_TYPES);
            //info.mAllowRoaming = getInt(Downloads.COLUMN_ALLOW_ROAMING) != 0;
            info.mTitle = getString(info.mTitle, PushSettings.BasePushColumns.COLUMN_TITLE);
            info.mDescription = getString(info.mDescription, PushSettings.BasePushColumns.COLUMN_DESCRIPTION);
            //info.mBypassRecommendedSizeLimit = getInt(Downloads.COLUMN_BYPASS_RECOMMENDED_SIZE_LIMIT);

            synchronized (this) {
                info.mControl = getInt(PushSettings.BasePushColumns.COLUMN_CONTROL);
            }
        }

        /**读取数据链接请求的头参数*/
        private void readRequestHeaders(PushInfo info) {
            info.mRequestHeaders.clear();
            Uri headerUri = Uri.withAppendedPath(info.getAllDownloadsUri(), PushSettings.RequestHeaders.URI_SEGMENT);
            Cursor cursor = mResolver.query(headerUri, null, null, null, null);
            try {
                int headerIndex = cursor.getColumnIndexOrThrow(PushSettings.RequestHeaders.COLUMN_HEADER);
                int valueIndex = cursor.getColumnIndexOrThrow(PushSettings.RequestHeaders.COLUMN_VALUE);
        ...
        private void addHeader(PushInfo info, String header, String value) {
        ...
        private String getString(String old, String column) {
            ...
            try {
                ...
                Log.i(TAG, "getString 数据库没有字段:"+column);
                return null;
            }
            if (old == null) {
        ...
        private Integer getInt(String column) {
            ...
            return mCursor.getInt(index);//mCursor.getColumnIndexOrThrow(column)
        }
    ...
    public String mPushNameCH;
    //public int mDestination;
    /**让通知栏是否显示下载进度的UI*/
    //public int mVisibility;
    
    public int mControl;
    /**下载的状态，是开始，还是在下载还是暂停 等等......*/
    public int mStatus;
    /**请求网络失败次数*/
    ...
    private PushInfo(Context context,Uri queryUri, SystemFacade systemFacade,PushTypeSharedPreferences pushTypeSharedPreferences) {
        mContext = context;
        mSystemFacade = systemFacade;
        mFuzz = PushHelpers.sRandom.nextInt(1001);
        mQueryUri = queryUri;
...
/*        if (mPackage == null) {
    ...
    public long restartTime(long now) {
        ...
        return mLastMod + PushConstants.RETRY_FIRST_DELAY * (1000 + mFuzz) * (1 << (mNumFailed - 1));
    }
    ...
    private boolean isReadyToStart(long now) {
        ...
        if (mControl == PushSettings.CONTROL_PAUSED) {
            // the download is paused, so it's not going to start
            Log.v("pushService", "the download is paused, so it's not going to start");
            return false;
        }
        Log.v("pushService", "isReadyToStart mStatus:" + mStatus + "    " + (restartTime(now) <= now));
        switch (mStatus) {
        case 0: // status hasn't been initialized yet, this is a new download
            case PushSettings.STATUS_PENDING: // download is explicit marked as
                                              // ready to start
            case PushSettings.STATUS_RUNNING: // download interrupted (process
                                              // killed etc) while
                                              // running, without a chance to update
                                              // the database
                return true;
    
            case PushSettings.STATUS_WAITING_FOR_NETWORK:
            case PushSettings.STATUS_QUEUED_FOR_WIFI:
                return checkCanUseNetwork() == NETWORK_OK;
    
            case PushSettings.STATUS_WAITING_TO_RETRY:
                // download was waiting for a delayed restart
                return restartTime(now) <= now;
           // default://这个是其他的状态 比如416 302之类的
             //   return true;
        }
        return false;
    }
     ...
     * completion.
     * 返回此下载是否完成后可见的通知。这里我们不需要，所有全部注释掉/后续需要在打开
     */
    public boolean hasCompletionNotification() {
/*        if (!PushSettings.isStatusCompleted(mStatus)) {
            return false;
        }
        if (mVisibility == PushSettings.VISIBILITY_VISIBLE_NOTIFY_COMPLETED) {
            return true;
        }*/
        return false;
    }
    ...
    private int checkSizeAllowedForNetwork(int networkType) {
        ...
        /*if (mBypassRecommendedSizeLimit == 0) {
            ...
            if (recommendedMaxBytesOverMobile != null && mTotalBytes > recommendedMaxBytesOverMobile) {
    ...
    void startIfReady(long now) {
        Log.i("pushService", "看看状态为 200 的会不会开始:"+mStatus+"    "+(!isReadyToStart(now)));
        if (!isReadyToStart(now)) {//mStatus 为200不会开始执行下面的下载线程
            return;
        }
        if (PushConstants.LOGVV) {
            Log.v(PushConstants.TAG, "Service spawning thread to handle download " + mId);
        }
        if (mHasActiveThread) {
            throw new IllegalStateException("Multiple threads on same download");
        }
        if (mStatus != PushSettings.STATUS_RUNNING) {
            mStatus = PushSettings.STATUS_RUNNING;
            ContentValues values = new ContentValues();
            values.put(PushSettings.BasePushColumns.COLUMN_STATUS, mStatus);
            mContext.getContentResolver().update(getAllDownloadsUri(), values, null, null);
            return;
        }
        PushThread downloader = new PushThread(mContext, mSystemFacade, this,mPushTypeSharedPreferences);
    ...
    public Uri getAllDownloadsUri() {
        ...
        Uri headerUri = Uri.withAppendedPath(mQueryUri, PushSettings.RequestHeaders.URI_SEGMENT);
        Log.e(TAG, "mQueryUri:"+mQueryUri+"        headerUri:"+headerUri);
        return ContentUris.withAppendedId(mQueryUri, mId);
    }


    public void logVerboseInfo() {
        Log.v(PushConstants.TAG, "Service adding new entry");
        Log.v(PushConstants.TAG, "ID      : " + mId);
        Log.v(PushConstants.TAG, "URI     : " + ((mUri != null) ? "yes" : "no"));
        //Log.v(PushConstants.TAG, "NO_INTEG: " + mNoIntegrity);
        Log.v(PushConstants.TAG, "HINT    : " + mHint);
        Log.v(PushConstants.TAG, "FILENAME: " + mFileName);
        Log.v(PushConstants.TAG, "MIMETYPE: " + mMimeType);
        //Log.v(PushConstants.TAG, "DESTINAT: " + mDestination);
        //Log.v(PushConstants.TAG, "VISIBILI: " + mVisibility);
        Log.v(PushConstants.TAG, "CONTROL : " + mControl);
        Log.v(PushConstants.TAG, "STATUS  : " + mStatus);
        Log.v(PushConstants.TAG, "FAILED_C: " + mNumFailed);
        Log.v(PushConstants.TAG, "RETRY_AF: " + mRetryAfter);
        Log.v(PushConstants.TAG, "LAST_MOD: " + mLastMod);
        //Log.v(PushConstants.TAG, "PACKAGE : " + mPackage);
        //Log.v(PushConstants.TAG, "CLASS   : " + mClass);
       // Log.v(PushConstants.TAG, "COOKIES : " + ((mCookies != null) ? "yes" : "no"));
       // Log.v(PushConstants.TAG, "AGENT   : " + mUserAgent);
       // Log.v(PushConstants.TAG, "REFERER : " + ((mReferer != null) ? "yes" : "no"));
        Log.v(PushConstants.TAG, "TOTAL   : " + mTotalBytes);
        Log.v(PushConstants.TAG, "CURRENT : " + mCurrentBytes);
        //Log.v(PushConstants.TAG, "ETAG    : " + mETag);
        Log.v(PushConstants.TAG, "DELETED : " + mDeleted);
    }
     ...
     * 
     * 返回的时间（从“现在”参数测量）的量在此下载将被激活。
     * 0 = 立即 - 服务应该坚持围绕处理此下载。
     * -1 = 永不 - 服务可以走而没有醒来。
     *正值 - 服务必须在未来醒来，从“现在”以毫秒为单位指定
     */
    long nextAction(long now) {
        if (PushSettings.isStatusCompleted(mStatus)) {
            return -1;
        }
        if (mStatus != PushSettings.STATUS_WAITING_TO_RETRY) {
            return 0;
        }

...
public class CameraTest extends TestBase implements SurfaceHolder.Callback {
	...
	private static final String CAMERA_IMAGE_BUCKET_NAME = Environment
			.getExternalStorageDirectory().toString() + "/CITTest/Camera/";
	...
	private void updatePreviewRectToCamera() {
		...
		Log.v("fengxuanyang", "loc[0]=" + loc[0] + " ---loc[1]=" + loc[1]); // 0
																			// 74
...
//			}
	...
	class AFMode0Thread extends Thread {
		public void run() {
			...
			mCanBack = true;
			Sleep(2000);
			Log.v(TAG, "mAFMode1FirstThread after Sleep(2000) mMode = " + mMode);

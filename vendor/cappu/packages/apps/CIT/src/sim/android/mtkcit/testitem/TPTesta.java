...
public class TPTesta extends Activity {
	TouchView touchview;
	class TouchView extends View {
		...
		private void touch_start(float f, float f1) {
			...
			Log.v("times", "touch_start_times=" + times);
			Log.v("CIT", "touch_start_f=" + f + "touch_start_f111=" + f1);
			if(times<0) {
				times=0;
			}
		...
		private void touch_up() {
			...
			float f = mX;
			float f1 = mY;
			Log.v("CIT", "touch_up_MX=" + f + "touch_up_MY=" + f1);
			Log.v("times", "touch_up_times=" + times);
			if (times < 0) {
				times = 0;
			}
			begin_end[times] = judgePoint(f, f1);
...
//			int m_height = screen_Y / 2;
//			int m_middle = 20;
			Log.v("fengxuanyang","screen_x"+screen_X+"screen_Y="+screen_Y );
			Log.v("CIT", "judgePoint__f1=" + f1 + "judgePoint__f2=" + f2);
		...
		private void judgePass(boolean[] bs, int j) {
			Log.v("CIT", "bs=BS.LENGTH=" + bs.length);
			times = 0;
			...
			for (int i = 0; i < graphics.size(); i++) {
				...
				Log.v("fengxuanyang", "mx=" + mx + "my=" + my);

				if (((-80f <= (3 * mx - 2 * my)) & ((3 * mx - 2 * my) <= 80f))
						| ((3*(screen_X-40f) <= (3 * mx + 2 * my)) & ((3 * mx + 2 * my) <= 3*(screen_X+40f)))) {
					mall++;
				}

			}
			Log.v("fengxuanyang", "graphics.size()==" + graphics.size());
			Log.v("fengxuanyang", "mall=" + mall + "pervent"
					+ ((float) mall / ((float) graphics.size())) * 100 + "%");
			if (((float) mall / ((float) graphics.size())) * 100 > 80) {
				AlertDialog.Builder builder = new AlertDialog.Builder(getContext())
						.setTitle(R.string.test_item_TP);
	...
	public void onClick(DialogInterface dialoginterface, int i) {
		...
		public TouchView(Context context) {
			...
			int i = TPTesta.screen_X;
			int j = TPTesta.screen_Y;
			Log.v("CIT", "screen_X=" + i + "screen_Y==" + j);

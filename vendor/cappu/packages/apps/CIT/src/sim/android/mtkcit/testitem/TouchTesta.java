...
public class TouchTesta extends Activity {
	...
	class TouchView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			Path path = mPath;
			Paint paint1 = TouchTesta.mPaint;
		...
		private boolean judgePoint(float f1, float f2) {
			...
			// int m_height = screen_Y / 2;
			// int m_middle = 20;
			Log.v("fengxuanyang", "screen_x" + screen_X + "screen_Y="
					+ screen_Y);
		...
		private void judgePass(boolean[] bs, int j) {
			...
			for (int i = 0; i < graphics.size(); i++) {
				...
				if (((-m_middle*4 <= (3 * mx - 2 * my)) & ((3 * mx - 2 * my) <= m_middle*4))
	...
	public void onClick(DialogInterface dialoginterface, int i) {
		...
		public TouchView(Context context) {
			...
			int i = TouchTesta.screen_X;
			int j = TouchTesta.screen_Y;

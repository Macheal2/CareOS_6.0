...
public class ProximityCalibration_tmd2771 extends TestBase implements
		Handler.Callback {
...
//			(new caThread()).start();
		...
		case 0:
			...
			if (calivalue[2] == 1) {
				try {
					...
					//citjni.PSwritenvram(calivalue[1],calivalue[0],calivalue[2]);
					Log.v(TAG, "PS_write_nvram far" + calivalue[1] + "close" + calivalue[0]
					+ "vaild" + calivalue[2]);
	...
	class calibratThread implements Runnable {
		...
		public void run() {
			...
			Log.v(TAG, "calivalue" + calivalue[0] + "calivalue" + calivalue[1]

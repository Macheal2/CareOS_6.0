...
public class FileViewInteractionHub implements IOperationProgressListener {
    ...
    private void notifyFileSystemChanged(String path) {
        ...
        /*ACTION_MEDIA_MOUNTED到了Android4.4就不灵了，Google将MEDIA_MOUNTED的权限提高了，于是就报了一个下面的错误。
        java.lang.SecurityException: Permission Denial: not allowed to send broadcast android.intent.action.MEDIA_MOUNTED from pid=18338, uid=10087*/
        if (f.isDirectory()) {
            ...
            Log.v("dengying", "path="+path+",directory changed, send broadcast:" + intent.toString());
        } else {
            intent = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE);

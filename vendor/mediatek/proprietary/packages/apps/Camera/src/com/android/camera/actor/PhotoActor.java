...
public class PhotoActor extends CameraActor implements FocusManager.Listener,
    ...
    private boolean mShutterSoundEnable=true;//dengjianzhang@20150611
    private boolean mTouchPhotographEnable=true; // added by bruce for touch photograph
    ...
    public boolean handleFocus() {
        ...
        boolean canAutoFocus = SettingUtils.isSupported(Parameters.FOCUS_MODE_AUTO,
                                mCameraActivity.getParameters().getSupportedFocusModes());
        if (mIsKeyHalfPressed && canAutoFocus) {
    ...
    public void onCameraClose() {
        ...
        mCameraActivity.getCameraDevice().setAutoFocusMoveCallback(null);
    ...
    private void initializeAfterPreview() {
        ...
        mShutterSoundEnable="on".equals(mCameraActivity.getISettingCtrl().getSettingValue(
        		SettingConstants.KEY_CAMERA_SHUTTER_SOUND));
        if(mShutterSoundEnable){
        	mCameraActivity.getCameraDevice().enableShutterSound();
        }else{
        	mCameraActivity.getCameraDevice().disenableShutterSound();
        }
        /**dengjianzhang@20150611 end*/

        // added by bruce for touch photograph begin
        mTouchPhotographEnable="on".equals(mCameraActivity.getISettingCtrl().getSettingValue(
        		SettingConstants.KEY_CAMERA_TOUCH_PHOTOGRAPH));
        if(mTouchPhotographEnable){
        	mCameraActivity.getCameraDevice().enableTouchPhotograph();
        }else{
        	mCameraActivity.getCameraDevice().disenableTouchPhotograph();
        }

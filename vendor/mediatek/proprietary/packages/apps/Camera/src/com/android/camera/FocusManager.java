...
public class FocusManager implements CameraActivity.OnOrientationListener,
    ...
    public void onAutoFocus(boolean focused) {
        ...
        // added by bruce for touch photograph begin
        boolean isEnable = SystemProperties.getInt("ro.touch_photograph_support", 0) == 1 ? true : false;
        boolean isOpen = "on".equals(mContext.getISettingCtrl().getSettingValue(
        		SettingConstants.KEY_CAMERA_TOUCH_PHOTOGRAPH));
        Log.i("bruce_nan", "onAutoFocus_nfl: isOpen = " + isOpen + "; mState = " + mState);
        
        if (isEnable && isOpen && mState == STATE_SUCCESS){
            doSnap();
        }
        // added by bruce for touch photograph end
    }
    ...
    private String getModeContinousFocusMode(int mode) {
        mode = getSettingModeIndex(mode);
		//dengjianzhang@20160104 add start
        return MATRIX_FOCUS_MODE_CONTINUOUS[mode];
        // In video mode, should set parameter focus mode FOCUS_MODE_AUTO
        //if (mode == ModePicker.MODE_LIVE_PHOTO || (mode == ModePicker.MODE_VIDEO && !isPdafSupported())
        //        || mode == ModePicker.MODE_VIDEO_PIP) {
        //    return null;
        //} else {
        //    return MATRIX_FOCUS_MODE_CONTINUOUS[mode];
        //}
		//dengjianzhang@20160104 add end
    }

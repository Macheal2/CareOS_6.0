    ...
    private static void updateNotification(
        ...
        if (uniqueThreadCount > 1 || mostRecentNotification.mThreadId <= 0) {    // messages from multiple threads
            ...
            if (FolderModeUtils.getMmsDirMode()) {
                ...
                //START: added by Yar @20170821
                intent = mostRecentNotification.mClickIntent;
                intent.addCategory(Intent.CATEGORY_LAUNCHER);
                intent.setAction(Intent.ACTION_MAIN);
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
                //END: added by Yar @20170821
        ...
        if (!sOpMessagingNotification.updateNotification(context, uniqueThreadCount,
        	...
        	noti.setContentIntent(PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_UPDATE_CURRENT));
        	//END: added by Yar @20170821
        ...
        if (isNew && !isContinuousComming) {
            ...
            */
            ...
            if (notiProf.needVibrate() 
               // && audioManager.shouldVibrate(AudioManager.VIBRATE_TYPE_NOTIFICATION)                //def vibarte true by zxy
               ){

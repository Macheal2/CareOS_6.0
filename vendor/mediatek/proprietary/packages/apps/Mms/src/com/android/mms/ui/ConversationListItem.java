...
// add by songkun for care style begin
    int[] contactImageIds = {R.drawable.ic_contact_picture_0,
                             R.drawable.ic_contact_picture_1,
                             R.drawable.ic_contact_picture_2,
                             R.drawable.ic_contact_picture_3,
                             R.drawable.ic_contact_picture_4,
                             R.drawable.ic_contact_picture_5};
    ...
    // add by songkun for care style end
    // M: add for op
	...
	mDraftView = (TextView)findViewById(R.id.draft_text);
	...
	// songkun add for Care Style @20150910
        mSelectView = (CheckBox)findViewById(R.id.select);
    }
...
//        LayoutParams attachmentLayout = (LayoutParams)mAttachmentView.getLayoutParams();
        ...
        if (!mIpConvListItem.onIpBind(mConversation.getIpConv(mContext), ((ConversationList) ConversationList.getContext()).isActionMode(), conversation.isChecked(), mConversation.getType(), this, mFromView, mSubjectView, mDateView)) {
            mFromView.setText(formatMessage());
			//added by songkun for Care Style @20150910
	    if(mConversation.hasDraft()){
	        mDraftView.setVisibility(VISIBLE);
	    }else{
	        mDraftView.setVisibility(GONE);
	    }
	    //end
            updateAvatarView();
            /// M:
 ...
 //       subjectLayout.addRule(RelativeLayout.LEFT_OF, hasAttachment ? R.id.attachment :
  //          (hasError ? R.id.error : R.id.date));
	...
	// add by songkun for Care style
        if (MmsConfig.getSelectStatus()){
	    Log.d("MessageEditDeleteActivity", "ConversationListItem_bind_songkun: MmsConfig.getSelectStatus true");
            mSelectView.setVisibility(View.VISIBLE);
            Log.d("MessageEditDeleteActivity", "ConversationListItem_bind_isChecked_songkun: " + isChecked());
            mSelectView.setChecked(isChecked());
            //mSelectView.toggle();
        }else {
	...
	//add by songkun for Care style
        mSelectView.setChecked(checked);

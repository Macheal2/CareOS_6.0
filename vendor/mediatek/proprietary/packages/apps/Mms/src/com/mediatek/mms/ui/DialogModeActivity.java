...
//added by yzs for I99 UI begin
import android.util.DisplayMetrics;
import android.view.ViewGroup.LayoutParams;
import android.view.LayoutInflater;
//aded by yzs end for I99 UI
...
public class DialogModeActivity extends Activity implements
    ...
    private com.android.mms.widget.ScrollingTextView mSender;
...
//        }
	...
	// initDislogSize();
	//modified by songkun for care begin
	LayoutInflater inflater = getLayoutInflater();
	mContentView = inflater.inflate(R.layout.msg_dlg_activity, null);
	setContentView(mContentView);

        final int width = getScreenSize()[0];
        final int height = getScreenSize()[1];
	LayoutParams lp = mContentView.getLayoutParams();
	if( width > height){
	   lp.width = getScreenSize()[0] / 2;
	}else{
	   lp.width = getScreenSize()[0] - 10;
        }
	mContentView.setLayoutParams(lp);
        //setContentView(R.layout.msg_dlg_activity);
	//modified by songkun for care end
	...
	mSender = (com.android.mms.widget.ScrollingTextView) findViewById(R.id.recepient_name);
	//modified by songkun for care end
...
//                (LinearLayout) this.findViewById(R.id.button_and_counter),
     ...
     //added by songkun for care begin
    public int[] getScreenSize() {
		DisplayMetrics dm = new DisplayMetrics();
		dm = DialogModeActivity.this.getResources().getDisplayMetrics();
    ...
    private void setDialogView() {
        ...
        if (msgNum <= 1) {
            ...
            //mMsgNumText.setVisibility(View.INVISIBLE);//songkun rm for care
        } else {
            mLeftArrow.setVisibility(View.VISIBLE);
            mRightArrow.setVisibility(View.VISIBLE);
            //mMsgNumText.setVisibility(View.VISIBLE);//songkun rm for care
            ...
            //mMsgNumText.setText(msgNumStr);//songkun rm for care
        }
        if (mReplyContent.size() != 0) {
            String content = mReplyContent.get(mCurUriIdx);
            mReplyEditor.setText(content);

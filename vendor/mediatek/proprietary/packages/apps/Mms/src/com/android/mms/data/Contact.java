...
//    };
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
         ...
         * }
         */
        Log.i(M_TAG, "formatNameAndNumber, name=" + name + ", number=" + number);
    ...
    public synchronized Uri getPhotoUri() {
        return Uri.withAppendedPath(getUri(), Contacts.Photo.DISPLAY_PHOTO);
    ...
    //added by songkun begin for care style @20150910
    public synchronized Drawable openPhoto(Context context, Drawable defaultValue, final long threadId){
        /*if(threadId > 0){
            Uri contactUri = getUri();
            Uri photoUri = Uri.withAppendedPath(contactUri, Contacts.Photo.CONTENT_DIRECTORY);//DISPLAY_PHOTO
            try{
                AssetFileDescriptor fd = context.getContentResolver().openAssetFileDescriptor(photoUri, "r");
                InputStream avatarDataStream = fd.createInputStream();
                byte[] avatarData = null;
                try{
                    if (avatarDataStream != null) {
                        avatarData = new byte[avatarDataStream.available()];
                        avatarDataStream.read(avatarData, 0, avatarData.length);
                    }
                    if (avatarData != null) {
                        Bitmap b = BitmapFactory.decodeByteArray(avatarData, 0, avatarData.length);
                        return new BitmapDrawable(context.getResources(), GetRoundedCornerBitmap(b));//zoomImage(b, 1.5, 1.5));
                    } 
                }finally {
                    try {
                        if (avatarDataStream != null) {
        ...
        if(threadId > 0){*/
        	...
        	Uri contactUri = getUri();
        	String temp1 = "content://com.android.contacts/contacts/-1";
			String temp2 = "content://com.android.contacts/contacts/-2";
			Uri temp0= Uri.parse("content://com.android.contacts/contacts/0");
        	if (contactUri.toString().equals(temp1) || contactUri.toString().equals(temp2)){
	    		contactUri = temp0;
        	}
            //Uri contactUri = getUri();
            ...
            try{
                InputStream avatarDataStream = ContactsContract.Contacts.openContactPhotoInputStream(cr, contactUri);
                byte[] avatarData = null;
    ...
    private Bitmap GetRoundedCornerBitmap(Bitmap bitmap) {
        try {
            ...
            final Rect rect = new Rect(0, 0, bitmap.getWidth(),
                    bitmap.getHeight());       
            final RectF rectF = new RectF(new Rect(0, 0, bitmap.getWidth() - 5, bitmap.getHeight() - 5));
            ...
            final Rect src = new Rect(10, 10, bitmap.getWidth(),
                    bitmap.getHeight());
    ...
    private static class ContactsCache {
        ...
        private void updateContact(final Contact c) {
            if (c == null) {
                /// M:
                MmsLog.d(TAG, "Contact.updateContact(): Contact for query is null.");
                return;
            }
...
//                /// @}
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(matchNumber)) {
                Cursor cursor = mContext.getContentResolver().query(
                        Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, normalizedNumber), CALLER_ID_PROJECTION, null, null, Phone.DISPLAY_NAME + " ASC");
                if (cursor == null) {
                    /// M:
        ...
        private byte[] loadAvatarData(Contact entry) {
            ...
            if ((!entry.mIsMe && entry.mPersonId == 0) || entry.mAvatar != null || !sNeedLoadAvatar) {
                /// M:
            ...
            try {
                /// M:
                MmsLog.d(TAG, "loadAvatarData(): avatarDataStream is null?=" + (avatarDataStream == null));
                if (avatarDataStream != null) {
                ...
                /// M:
                MmsLog.w(TAG, "loadAvatarData(): IOException!");
            } finally {
                try {
                    if (avatarDataStream != null) {
                        avatarDataStream.close();
        ...
        //synchronized (ContactsCache.this) {
                ...
                workingNumberOrEmail = workingNumberOrEmail.replaceAll(" ", "").replaceAll("-", "").replaceAll("[()]", "");
...
//        /// M: add for group contact
    ...
    public void setContactInfoFormChipWatcher(String name, String number, long personId, byte[] data) {
        mName = name;
        setNumber(number);
        mAvatarData = data;
        mAvatar = null;

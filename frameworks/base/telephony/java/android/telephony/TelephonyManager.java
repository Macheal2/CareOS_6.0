...
public class TelephonyManager {
    ...
    */
     ...
     * @hide
     */
    public int getTetherApnRequired(int subId) {
        try {
            ITelephony telephony = getITelephony();
            if (telephony != null)
                return telephony.getTetherApnRequired(subId);
        } catch (RemoteException ex) {
    ...
    public void setDataEnabled(int subId, boolean enable) {
       ...
       ///zhaogangzhu@20151013 dataconn secretcode Broadcast
		boolean isEnable=false ;
		boolean isDataon=false ;
		if(mContext != null){
		isEnable = Settings.Global.getInt(mContext.getContentResolver(),"enable_data_conn_secretcode", 1)==1;
		isDataon = Settings.Global.getInt(mContext.getContentResolver(),Settings.Global.MOBILE_DATA, 1) == 1;
		}
        try {
            Log.d(TAG, "setDataEnabled: enabled=" + enable);
            ITelephony telephony = getITelephony();
			//hejianfeng add start
			if(mContext != null && !enable){
				Log.d(TAG, "jeff android.net.conn.CONNECTIVITY_CANCEL" + enable);
			...
			if(isEnable){
				if(!isDataon ){
                     if(mContext != null){
						Intent intent = new Intent("android.net.conn.CAREDIALOGSHOW");
						mContext.sendBroadcast(intent);
	                    }else{
	                    Log.i("zhaogangzhu","setDataEnabled mContext is null can't sendBroadcast CAREDIALOGSHOW ");
	                    }
				}else{
					if(telephony!=null){

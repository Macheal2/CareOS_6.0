...
#define SYS_CAPPU_PACKAGE_ID      0x03
    ...
    if (*s == '@') {
        ...
        if (len == 5 && s[1]=='n' && s[2]=='u' && s[3]=='l' && s[4]=='l') {
            ...
            if (rid != 0) {
                ...
                // add by y.haiyang SYS_CAPPU_PACKAGE_ID
                if (packageId != APP_PACKAGE_ID && packageId != SYS_PACKAGE_ID && packageId != SYS_MTK_PACKAGE_ID && packageId != SYS_CAPPU_PACKAGE_ID) {
            ...
            if (accessor) {
                ...
                if (rid != 0) {
                    ...
                    if (packageId == 0x00) {
                        ...
                        outValue->dataType = Res_value::TYPE_DYNAMIC_REFERENCE;
                        return true;
                    // add by y.haiyang for SYS_CAPPU_PACKAGE_ID
                    } else if (packageId == APP_PACKAGE_ID || packageId == SYS_PACKAGE_ID || packageId == SYS_MTK_PACKAGE_ID || packageId == SYS_CAPPU_PACKAGE_ID) {
    ...
    : mAssignedPackageId(packageId)
{
    ...
    // add by y.haiyang for cappu-res
    mLookupTable[SYS_CAPPU_PACKAGE_ID] = SYS_CAPPU_PACKAGE_ID;
}

...
public class LockPatternUtils {
    ...
    public void setDuplicateLockScreenEnabled(boolean enabled) {
        if (enabled) {
            Settings.Secure.putInt(mContext.getContentResolver(), "duplicate_unlock_enabled", 1);
        } else {
            Settings.Secure.putInt(mContext.getContentResolver(), "duplicate_unlock_enabled", -1);
        }
    }

    public boolean getDuplicateLockScreenEnabled() {
        try {
            return Settings.Secure.getInt(mContext.getContentResolver(), "duplicate_unlock_enabled", 1) == 1;
        } catch (Exception e) {
        }
        return false;
    }

    public void setDuplicateLockStyle(int style) {
        Settings.Secure.putInt(mContext.getContentResolver(), "duplicate_unlock_style", style);
    }

    public int getDuplicateLockStyle() {
        try {
            return Settings.Secure.getInt(mContext.getContentResolver(), "duplicate_unlock_style", 0);
        } catch (Exception e) {
        }
        return 0;
    }
    ...
    public boolean isLockScreenDisabled(int userId) {
        ...
        if(getDuplicateLockScreenEnabled()){
           return false;
        }
    ...
    public void clearVoiceUnlock() {
        ...
        Settings.System.putStringForUser(mContentResolver, Settings.System.VOICE_UNLOCK_SCREEN,
                null, ActivityManager.getCurrentUser());
        Settings.System.putStringForUser(mContentResolver,
                Settings.System.VOICE_UNLOCK_AND_LAUNCH1, null, ActivityManager.getCurrentUser());
        Settings.System.putStringForUser(mContentResolver,
                Settings.System.VOICE_UNLOCK_AND_LAUNCH2, null, ActivityManager.getCurrentUser());
        Settings.System.putStringForUser(mContentResolver,
                Settings.System.VOICE_UNLOCK_AND_LAUNCH3, null, ActivityManager.getCurrentUser());
    }

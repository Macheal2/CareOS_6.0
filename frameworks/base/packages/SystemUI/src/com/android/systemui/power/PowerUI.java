    ...
    private final class Receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
                ...
                //modified by yzs for play lowbattery sound begin
                if (!plugged
                        && (mBatteryLevel == 5 || ((bucket < oldBucket || oldPlugged) //dengying@20160630
                        && mBatteryStatus != BatteryManager.BATTERY_STATUS_UNKNOWN
                        && bucket < 0))){
                    // only play SFX when the dialog comes up or the bucket changes
                    final boolean playSound = bucket != oldBucket || oldPlugged;
                    //mWarnings.showLowBatteryWarning(playSound);
                    mWarnings.showLowBatteryWarning(!mPowerManager.isPowerSaveMode());

                } else if (plugged || (bucket > oldBucket && bucket > 0) || (mBatteryLevel > 5)) {//dengying@20160630
                //modified by yzs for play lowbattery sound end

...
public class KeyguardViewMediator extends SystemUI {
	...
	private boolean mIsMagcommLockerShowing = false;
    ...
    ViewMediatorCallback mViewMediatorCallback = new ViewMediatorCallback() {
        ...
        // added by yzs for MagcommLocker begin
        @Override
        public void setMagcommLockerShowing(boolean isShowing) {
            mIsMagcommLockerShowing = isShowing;
            KeyguardViewMediator.this.updateMagcommLockerShowingState();
        }

        @Override
        public boolean isMagcommLockerShowing() {
            return KeyguardViewMediator.this.isMagcommLockerShowing();
        }
        // added by yzs for MagcommLocker end
    ...
    //added by yzs for MagcommLocker begin
    public boolean isMagcommLockerShowing() {
        return mIsMagcommLockerShowing;
    }

    public void updateMagcommLockerShowingState() {
        try {
            int size = mKeyguardStateCallbacks.size();
            for (int i = 0; i < size; i++) {
                mKeyguardStateCallbacks.get(i).onMagcommLockerShowingStateChanged(mIsMagcommLockerShowing);
            }
        } catch (RemoteException e) {
            Slog.w(TAG, "Failed to call updateMagcommLockerShowingState", e);
        }
    }
    //added by yzs for MagcommLocker end
    ...
    private void handleHide() {
        synchronized (KeyguardViewMediator.this) {
            ...
            mHiding = true;
            //modified by yzs for MagcommLocker begin
            if (!mIsMagcommLockerShowing && mShowing && !mOccluded) {
            //if (mShowing && !mOccluded) {
            //modified by yzs for MagcommLocker end
    ...
    public void addStateMonitorCallback(IKeyguardStateCallback callback) {
        synchronized (this) {
            ...
            try {
                ...
                callback.onMagcommLockerShowingStateChanged(mIsMagcommLockerShowing);//added by yzs for MagcommLocker 
            } catch (RemoteException e) {

...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
	...
	//added by yzs for MagcommLocker begin
	private View mDuplicateLockScreen = null;
    ...
    /*begin: edit by hmq 20151109 Modify for launcher状态栏透明*/
    private final static String LAUNCHER_ACTION = "com.cappu.launcher.pagechange";
    /*end: edit by hmq 20151109 Modify for launcher状态栏透明*/
    /*begin: edit by hmq 20160517 Modify for 来电禁止下拉状态栏*/
    private boolean mIsCallInComming;
    private boolean mIsInCallUIForeground;
    /*end: edit by hmq 20160517 Modify for 来电禁止下拉状态栏*/
    ...
    NotificationPanelView mNotificationPanel; //hmq view root // the sliding/resizing panel within the notification window
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        mLockPatternUtils = new LockPatternUtils(mContext);//added by yzs for MagcommLocker
        ...
        /*end: edit by hmq 20151109 Modify for launcher状态栏透明*/
        filter.addAction("com.android.incallui.IsForeground");//edit by hmq 20160517 Modify for 来电禁止下拉状态栏
        ...
        //获取电话通讯服务  
       TelephonyManager tpm = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);  //add by hmq 20160504状态栏分2页显示
       //创建一个监听对象，监听电话状态改变事件  
       tpm.listen(new MyPhoneStateListener(), PhoneStateListener.LISTEN_CALL_STATE); //add by hmq 20160504状态栏分2页显示
    ...
    /**
     * edit by hmq 20160504状态栏分2页显示
     * 公开dismiss clear all Notification
     */
    public void activityClearAllNotifications() {
        clearAllNotifications();
    }
    ...
    private void updateQsExpansionEnabled() {
        mNotificationPanel.setQsExpansionEnabled(isDeviceProvisioned()
                ...
                && !ONLY_CORE_APPS
				&& mDuplicateLockScreen == null//hmq magcomm add <mDuplicateLockScreen == null> 当magcomm lock screen View不为空 禁止状态栏下拉
				);
    }
    ...
    private void updateClearAll() {
        ...
        //mStackScroller.updateDismissView(showDismissView);//hide by hmq 20160504状态栏分2页显示 不去更新DismissView的visibility
        mNotificationPanel.updateDismissView(showDismissView);//add by hmq 20160504状态栏分2页显示 更新header SwitchPageButton的enabled
    }
    ...
    public void showClock(boolean show) {
        if (mIconController == null) return;
        mIconController.setClockVisibility(show);
    }
    ...
    public void disable(int state1, int state2, boolean animate) {
        /*begin: edit by hmq 20160517 Modify for 来电禁止下拉状态栏*/
        if(mIsCallInComming && mIsInCallUIForeground){
    ...
    public void setLightsOn(boolean on) {
        ...
        /* begin: edit by hmq 2016912 Modify for launcher状态栏锁屏消息提示时(QQ)禁止下载状态栏 */
        if (on) {
            ActivityManager am = (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
            String runningActivity = am.getRunningTasks(1).get(0).topActivity.getClassName();
            if (runningActivity.equals("com.tencent.mobileqq.activity.QQLSActivity")) {
                mDisabledUnmodified1 |= StatusBarManager.DISABLE_EXPAND;
                disable(mDisabledUnmodified1,mDisabledUnmodified2, false /* animate */);
            }
        }
        /* end: edit by hmq 2016912 Modify for launcher状态栏锁屏消息提示时(QQ)禁止下载状态栏 */
        if (on) {
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            /*begin: edit by hmq 20151109 Modify for launcher状态栏透明*/
            boolean isLauncher = LAUNCHER_ACTION.equals(action);
            int isLauncherHidePage = isLauncher ? intent.getIntExtra("page", -1) : -1;
            /*end: edit by hmq 20151109 Modify for launcher状态栏透明*/
            ...
            /*begin: edit by hmq 20160517 Modify for 来电禁止下拉状态栏 这个广播发送方InCallActivity.java*/
            if ("com.android.incallui.IsForeground".equals(action)) {
                mIsInCallUIForeground = intent.getBooleanExtra("status", false);
                if (mIsCallInComming && !mIsInCallUIForeground && (mDisabled1 & StatusBarManager.DISABLE_EXPAND) != 0) {
                    mDisabled1 &= ~StatusBarManager.DISABLE_EXPAND;
                    mDisabledUnmodified1 &= ~StatusBarManager.DISABLE_EXPAND;
                }else if (mIsCallInComming && mIsInCallUIForeground){
                    onBackPressed();
                }
            }
            /*end: edit by hmq 20160517 Modify for 来电禁止下拉状态栏*/
   ...
   // added by yzs for MagcommLocker begin
    private void creatAndAddDuplicateUnlockView() {//dengying@20160822 lockscreen
        try {
            Configuration mConfiguration = mContext.getResources().getConfiguration();
            if (0 == mLockPatternUtils.getDuplicateLockStyle()) {
                mDuplicateLockScreen = new MagcommBootUnlockView(mContext, mConfiguration);
            } else {
                mDuplicateLockScreen = new CappuBootUnlockView(mContext, mConfiguration);//dengying@20160822 lockscreen
            }
        } catch (Exception e) {
            e.printStackTrace();
            Log.i("yzs002", " error is called and e = " + e);
            mDuplicateLockScreen = null;
        }
        if (mDuplicateLockScreen != null) {

            mNotificationPanel.addView(mDuplicateLockScreen, 0);
            //mNotificationPanel.setQsExpansionEnabled(false);
            RotationPolicy.setRotationLock(mContext, true);
            if (0 == mLockPatternUtils.getDuplicateLockStyle()) {
                ((MagcommBootUnlockView) mDuplicateLockScreen).setViewMediatorCallback(mKeyguardViewMediatorCallback);
                mNotificationPanel.setMagcommLockerView((MagcommBootUnlockView) mDuplicateLockScreen);
            } else {
                ((CappuBootUnlockView) mDuplicateLockScreen).setViewMediatorCallback(mKeyguardViewMediatorCallback);
                mNotificationPanel.setCappuLockerView((CappuBootUnlockView) mDuplicateLockScreen);
            }
        }
    }

    public void destroyMagcommLocker() {
        if (mDuplicateLockScreen != null) {
            if (0 == mLockPatternUtils.getDuplicateLockStyle()) {
                ((MagcommBootUnlockView) mDuplicateLockScreen).onDestroy();
                mNotificationPanel.setMagcommLockerView(null);
            } else {
                ((CappuBootUnlockView) mDuplicateLockScreen).onDestroy();
                mNotificationPanel.setCappuLockerView(null);
            }
            // mDuplicateLockScreen.onDestroy();
            RotationPolicy.setRotationLock(mContext, mRotation);
            //mNotificationPanel.setQsExpansionEnabled(true);
            mNotificationPanel.removeView(mDuplicateLockScreen);
            mDuplicateLockScreen = null;
        }
    }
    //added by yzs for MagcommLocker end
    ...
    public void showKeyguard() {
        // added by yzs for MagcommLocker begin
        mRotation = RotationPolicy.isRotationLocked(mContext);
        if (mDuplicateLockScreen == null) {
            if (mLockPatternUtils.getDuplicateLockScreenEnabled()) {
                destroyMagcommLocker();
                creatAndAddDuplicateUnlockView();// dengying@20160822 lockscreen
                if (mKeyguardViewMediatorCallback != null) {
                    mKeyguardViewMediatorCallback.setMagcommLockerShowing(true);
                }
            }

            if (mDuplicateLockScreen == null) {
                if (mKeyguardViewMediatorCallback != null) {
                    mKeyguardViewMediatorCallback.setMagcommLockerShowing(false);
                }
            }
            setShowLockscreenNotifications(false);
        }
        // added by yzs for MagcommLocker end
        if (mLaunchTransitionFadingAway) {
    ...
    public boolean hideKeyguard() {
        ...
        destroyMagcommLocker();//added by yzs for MagcommLocker 
    ...
    public boolean onBackPressed() {
        ...
        //hide by hmq 20160504状态栏分2页显示 hide the original code 这段代码的意思是点击返回键隐藏QS
...
//        }
    ...
    public boolean onDraggedDown(View startingChild, int dragLengthY) {
        if (hasActiveNotifications()) {
            ...
            goToLockedShade(startingChild); //yzs
            return true;
        } else {

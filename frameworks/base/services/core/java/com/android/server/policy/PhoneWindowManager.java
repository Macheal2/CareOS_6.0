...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private void handleLongPressOnHome(int deviceId) {
        if (mLongPressOnHomeBehavior != LONG_PRESS_HOME_NOTHING) {
			...
			// dengying home_key sos begin
			boolean b_sos_key_state = false;
			...
			Log.v("dengyingKey", "handleLongPressOnHome b_sos_key_state=" + b_sos_key_state+",mIsAlarmBoot="+mIsAlarmBoot);
			...
			if (b_sos_key_state) {
				if (!mIsAlarmBoot) {
					...
					for (RunningTaskInfo info : list) {
						if (info.topActivity.getPackageName().equals("cappu.sos") && info.baseActivity.getPackageName().equals("cappu.sos")) {
							isAppRunning = true;
							break;
						}
					}
					...
					if (isAppRunning) {
						am.forceStopPackage("cappu.sos");
					}

					Log.v("dengyingHomeKey", "come into SosShowActivity");

					Intent mIntent = new Intent();
					mIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					ComponentName comp_SOS = new ComponentName("cappu.sos", "cappu.sos.SOSShowActivity");
					mIntent.setComponent(comp_SOS);
					mContext.startActivity(mIntent);
				}
			}
			// dengying home_key sos end
    ...
    private boolean isInCallUI() {
        ...
        Log.e("Yar", " pkg = " + currentPackageName + " ClassName=" + currentClassName + " ShortClassName = " + currentShortClassName);
             
        // LED pulse when screenOn only if LEDVibraTest is running 
        if (!currentPackageName.equals("") && !currentClassName.equals("") && currentClassName.equals("com.android.incallui.InCallActivity")){
            return true;
        }    

        return false;                                                                                                                                                                                       
    }
    //END: added by Yar @20170822
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if ((false == IS_USER_BUILD) || DEBUG_INPUT) {
            ...
            Log.d("dengyingKey", "interceptKeyTi keyCode=" + keyCode + " down=" + down + " repeatCount="
                    + repeatCount + " keyguardOn=" + keyguardOn + " mHomePressed=" + mHomePressed
                    + " canceled=" + canceled + " metaState:" + metaState);
        }
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            // added by bruce for dispatch the home key to CIT key test begin
            Log.i("bruce_nan", "interceptKeyBeforeDispatching_nfl: package = " + win.getOwningPackage());
            if((win != null) && (win.getAttrs() != null) && win.getOwningPackage().equals("sim.android.mtkcit")){
                return 0;// return 0 indicate dispatch the home key to app
              }
            // added by bruce for dispatch the home key to CIT key test end

            // caiji add home key notice for incallui (start)
            if((win != null) && (win.getAttrs() != null) && (win.getOwningPackage().equals("com.android.dialer")
	            && isInCallUI())){
                return 0;// return 0 indicate dispatch the home key to app
            }
            // caiji add home key notice for incallui (end)        	
        	
            //modify by even 20160630
            //when isScreenOn() = false ,at the same time isKeyguardLocked() = false, homekey only turn screen on.
            //start 
            Log.i("so7799", "interceptKeyBeforeDispatching  isScreenOn() = " + isScreenOn() +" ," + " isKeyguardLocked = " + isKeyguardLocked());
            ...
            if (attrs != null) {
                ...
                if (type == WindowManager.LayoutParams.TYPE_KEYGUARD_SCRIM
					...
					if (!b_sos_key_state) {// dengying home_key sos
						return 0;
					}
				}
			...
			} else if (repeatCount == 15/*(event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0 */) {
				if (!keyguardOn || b_sos_key_state) {// dengying home_key sos

					Log.e("dengyingKey","interceptKeyTi b_sos_key_state="+b_sos_key_state);
					
					if (b_sos_key_state) {
						int is_home_key_wake_up = Settings.Global.getInt(resolver, "home_key_wake_up", 1);
						boolean b_home_key_wake_up = (is_home_key_wake_up == 1) ? true : false;

						if (!b_home_key_wake_up) {
			...
			} else if ((event.getFlags() & KeyEvent.FLAG_LONG_PRESS) != 0) {// dengying home_key  menu
				Log.v("dengyingHomeKey", "menu long press==>home");
	...
	//modify by even hejianfeng@20150601 for hall
    public static boolean mHall_7799 = false;
    /** {@inheritDoc} */
    @Override
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        final boolean keyguardActive = (mKeyguardDelegate == null ? false :
                                            (interactive ?
                                                ...
                                                mKeyguardDelegate.isShowing()));
			//modify by even hejianfeng@20150601 hall
		int isHallMenuChecked = 0;
		final int repeatCount = event.getRepeatCount();
		//hejianfeng@20150601 end
        /// M: Remove this log.
        if (false && DEBUG_INPUT) {
            ...
            Log.d("dengyingKey", "interceptKeyTq keycode=" + keyCode
                    + " interactive=" + interactive + " keyguardActive=" + keyguardActive
                    + " policyFlags=" + Integer.toHexString(policyFlags));
        }
        ...
        // Handle special keys.
        switch (keyCode) {
		//modify by even hejianfeng@20150601 start hall
	    case KeyEvent.KEYCODE_HALL:		    
		    Log.d("even7799", "interceptKeyTq switch keyCode=KEYCODE_HALL");
		    //is hall menu checked?		    
		    isHallMenuChecked = Settings.Global.getInt(mContext.getContentResolver(), Settings.System.HALL_SETTING, 0);
		    if(0 == isHallMenuChecked){
				Log.d("even7799","*****************hall menu not checked***************************");
				break;
		    }else {
		    	isWakeKey = true;
		    	Log.d("even7799","*****************hall menu checked***************************");
			}
		    
		    if (down && repeatCount == 0) {
				Log.d("even7799","***************downdown********************mHall_7799==true********");
				mHall_7799 = true;
				Settings.Global.putInt(mContext.getContentResolver(), Settings.System.HALL_CLOSE, 1);
	
				boolean incomingRinging = false;
				boolean isAnswerRingCall = false;
				boolean isRinging = false;
	
				try {
					TelecomManager telecomManager = getTelecommService();
					if (telecomManager != null) {
						isAnswerRingCall = telecomManager.isInCall();
						isRinging = telecomManager.isRinging();
						incomingRinging = isAnswerRingCall||isRinging;
					}
				} catch (Exception ex) {
					Log.w(TAG, "RemoteException from getPhoneInterface()", ex);
				}
	
				if (incomingRinging || (!mSystemReady)) {
					Log.i("even7799", "Ignoring HOME; there's a ringing incoming call.");
					break;
				} else {
					Log.d("even7799","***************downdown********************startActivity intent********");
					Intent mIntent = new Intent("magcomm.action.LID_ON");
					mContext.sendBroadcast(mIntent);	 
				}               
		   }
		 break;
	     case KeyEvent.KEYCODE_HALL_RELEASE:
		    ...
		    if(down && repeatCount == 0){
				Log.d("even7799","***************upup********************mHall_7799=false********");
				mHall_7799 = false;
				Settings.Global.putInt(mContext.getContentResolver(), Settings.System.HALL_CLOSE, 0);
				/** hejianfeng delete 
				boolean incomingRinging = false;
				boolean isAnswerRingCall = false;
				boolean isRinging = false;
				...
				if (!mSystemReady) {
					...
					Log.d("even7799","***************upup********************close activity********");
					Intent mIntent = new Intent("magcomm.action.LID_OFF");
	                mContext.sendBroadcast(mIntent);
	                break;			
				}
		    }
		break;
	    // hejianfeng@20150601 end
		// added by dengying for home_key sos
		case KeyEvent.KEYCODE_HOME:
			Log.d("dengyingKey", "interceptKeyTq(interceptKeyBeforeQueueing) case KeyEvent.KEYCODE_HOME");

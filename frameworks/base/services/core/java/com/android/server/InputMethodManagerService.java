...
public class InputMethodManagerService extends IInputMethodManager.Stub
        ...
        synchronized (mMethodMap) {
            ...
            if (!mSystemReady) {
                ...
                if (!mImeSelectedOnBoot) {
                  ...
                  String preInstalledImeName = "com.baidu.input_spec.ImeService";
    ...
    private void updateSystemUiLocked(IBinder token, int vis, int backDisposition) {
        ...
        try {
            ...
            if (imi != null && needsToShowImeSwitcher) {
				...
				// add by hejianfeng for c687 begin
                final Intent intent = new Intent("com.cappu.action.KEYBOARD_HIDE_SHOW");
                intent.putExtra("KEYBOARD_STATE", mNotificationShown);
                mContext.sendBroadcast(intent);
                // add by hejianfeng for c687 end
        } finally {
            Binder.restoreCallingIdentity(ident);
        }
    }
    ...
    private class SwitchImeTask extends java.util.TimerTask {
        public void run() {
            synchronized (mMethodMap) {
                if (index2 == index1) {
                    final List<ImeSubtypeListItem> imList = mSwitchingController.getSortedInputMethodAndSubtypeListLocked(true, false, false);

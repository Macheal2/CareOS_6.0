...
public class PackageManagerService extends IPackageManager.Stub {
    ...
    public PackageManagerService(Context context, Installer installer,
        ...
        Log.d("debug_default","PackageManager is called.");
        setDefaultBrowser();
    }
	...
	private final void setDefaultBrowser() {
		...
		// set QQBrowser，if your want set other browser as default, modify below
		// code, add package name and activiy name
		ComponentName component = new ComponentName("com.tencent.mtt", "com.tencent.mtt.MainActivity");
			
		// find all browsers
		List<ResolveInfo> resolveInfoList = queryIntentActivities(intent,
				intent.getType(), PackageManager.GET_INTENT_FILTERS, 0);
		int size = resolveInfoList.size();
		ComponentName[] arrayOfComponentName = new ComponentName[size];
		// make sure the default browser in phone
		boolean no_QQBrowser = true;
		for (int i = 0; i < size; i++) {
			ActivityInfo activityInfo = resolveInfoList.get(i).activityInfo;
			String packageName = activityInfo.packageName;
			String className = activityInfo.name;
			// if your want set other browser as default, modify below package
			// name
		...
		//	Log.d("debug_default", i + ". arrayOfComponentName[" + i + "] = " + arrayOfComponentName[i]);
		}
		
		if (no_QQBrowser) {
			Log.d("debug_default", "no QQBrowser package");
			return;
		} else {
			Log.d("debug_default", "QQBrowser package is in, set it as Preferred.");
		}
		// set default
		addPreferredActivity2(filter, IntentFilter.MATCH_CATEGORY_SCHEME,
				arrayOfComponentName, component);
		Log.d("debug_default", "add QQBrowser to PreferredActivity = " + IntentFilter.MATCH_CATEGORY_SCHEME);
	}
	...
	public void addPreferredActivity2(IntentFilter filter, int match, 
		...
		synchronized (mPackages) {
			...
			Log.d("debug_default", "synchronized is called.");

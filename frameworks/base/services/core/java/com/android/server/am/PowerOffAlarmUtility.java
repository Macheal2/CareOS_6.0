...
public class PowerOffAlarmUtility {
    ...
    public static PowerOffAlarmUtility getInstance(Context ctx,
            ActivityStack aStack) {
        if (mInstance != null) {
            return mInstance;
        }
        if (ctx != null && aStack != null) {
            mInstance = new PowerOffAlarmUtility(ctx, aStack);
        }
        return mInstance;
    }
    ...
    private final void registerNormalBootReceiver(Context context) {
        ...
        //modify by even
        filter.addAction(ACTION_REBOOT_EVEN);
                ...
                if ("android.intent.action.normal.boot".equals(action)) {
                    ...
                    if (mFirstBoot) {
                        ...
                        synchronized (mStack.mService) {
                            mStack.mService.setBootingVal(true);
                            mStack.resumeTopActivityLocked(null);
                        }
                    } else {
                        mStack.mService.ipoBootCompleted();
                    }
                } else if (ACTION_REBOOT_EVEN.equals(action)) {
                    synchronized (mStack.mService) {
                    Log.w(TAG, "ALARM_BOOT_DONE normally shutdowns device 444444");
                    mStack.mService.rebootSystem("android.intent.action.normal.boot");//modify by even
                    }

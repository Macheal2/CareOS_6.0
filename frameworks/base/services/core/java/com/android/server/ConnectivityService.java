...
//zhaogangzhu@20151013 dataconn secretcode begin
...
//zhaogangzhu@20151013 dataconn secretcode end
...
public class ConnectivityService extends IConnectivityManager.Stub
    ...
    private final static int MIN_NET_ID = 1000; // some reserved marks
...
filterC.addAction("android.net.conn.CAREDIALOGSHOW");//zhaogangzhu@20151013 add
		...
		mTelephonyManager.listen(new MyPhoneStateListener(), PhoneStateListener.LISTEN_CALL_STATE);//zhaogangzhu@20151013 add
    }
    ...
    @VisibleForTesting
    protected int reserveNetId(NetworkCapabilities nc) {
        /** @} */
    ...
    public NetworkInfo getActiveNetworkInfo() {
        ...
        if (DBG) log("getActiveNetworkInfo networkInfo = " + state.networkInfo);
    ...
    public Network getActiveNetwork() {
        ...
        /** M: add log for debugging */
        if (DBG) log("getActiveNetwork nai = " + (nai == null ? "null" : nai.name()));
	...
	//zhaogangzhu@20151013 add dataconn secretcode begin@{
    public void careSetMobileDataEnabled(boolean enabled) {
	...
	//zhaogangzhu@20151013 add dataconn secretcode end@}
                ...
                case NetworkAgent.EVENT_NETWORK_CAPABILITIES_CHANGED: {
                    ...
                    if (nai == null) {
                        ...
                        if (networkCapabilities.hasCapability(NET_CAPABILITY_CAPTIVE_PORTAL) ||
                            ...
                            loge("BUG: " + nai + " has CS-managed capability.");
                        }
                        if (nai.created && !nai.networkCapabilities.equalImmutableCapabilities(
                                networkCapabilities)) {
                            loge("BUG: " + nai + " changed immutable capabilities: "
    ...
    private void handleSetAcceptUnvalidated(Network network, boolean accept, boolean always) {
        ...
        if (!nai.networkMisc.explicitlySelected) {
            loge("BUG: setAcceptUnvalidated non non-explicitly selected network");
        }
    ...
    private void setProvNotificationVisibleIntent(boolean visible, int id,
        ...
        if (visible) {
            ...
            if (notifyType == NotificationType.NO_INTERNET &&
                ...
                loge("Unknown notification type " + notifyType + "on network type "
    ...
    public int registerNetworkAgent(Messenger messenger, NetworkInfo networkInfo,
        ...
        final NetworkAgentInfo nai = new NetworkAgentInfo(messenger, new AsyncChannel(),
                new Network(reserveNetId(networkCapabilities)), new NetworkInfo(networkInfo),
                new LinkProperties(linkProperties), new NetworkCapabilities(networkCapabilities),
                currentScore, mContext, mTrackerHandler, new NetworkMisc(networkMisc),
...
//        }
    ...
    private void updateDnses(LinkProperties newLp, LinkProperties oldLp, int netId,
        ...
        if (oldLp == null || (newLp.isIdenticalDnses(oldLp) == false)) {
            ...
            if (useDefaultDns && mDefaultDns != null && !dnses.contains(mDefaultDns)) {
                dnses = new ArrayList(dnses);
                dnses.add(mDefaultDns);

                if (DBG) {
                    loge("Add google dns provided for " + netId + " - using "
                            + mDefaultDns.getHostAddress());
                }
            }
    ...
    private void updateCapabilities(NetworkAgentInfo nai, NetworkCapabilities networkCapabilities) {
        ...
        if (!Objects.equals(nai.networkCapabilities, networkCapabilities)) {
            ...
            ///M: Check Internet capability with default request.
            rematchCapabilityClearDefault(nai, networkCapabilities);
            ...
            if (satisfies) {
                ...
                if (currentNetwork == null ||
                    ...
                    if (!newNetwork.addRequest(nri.request)) {
                        loge("BUG: " + newNetwork.name() + " already has " + nri.request);
                    }
                ...
                if (currentNetwork == newNetwork) {
                    ...
                    if (nri.isRequest == true) {
                        loge("BUG: Removing request " + nri.request.requestId + " from " +
                                newNetwork.name() +
    ...
    private void updateNetworkInfo(NetworkAgentInfo networkAgent, NetworkInfo newInfo) {
        ...
        if (networkAgent.networkCapabilities.hasCapability(
            ...
            sendStickyBroadcast(new Intent("action.mediatek.mms.connectivity"));
        }
        ...
        if (state == NetworkInfo.State.CONNECTED && !networkAgent.created) {
            ...
            /** M: avoid the network can't reach google server in some regions.
             * don't prompt the dialog to confuse user.
             */
...
//        }
	...
	class MyPhoneStateListener extends PhoneStateListener {  
		...
		public void onCallStateChanged(int state, String incomingNumber) {
			...
			case TelephonyManager.CALL_STATE_RINGING: //来电	
				if(mTelephonyManager != null){
					if(mCareDialog !=null){
						if(mCareDialog.isShowing()){
							mCareDialog.dismiss();
							}
						}
					Log.i("zhaogangzhu","MyPhoneStateListener  get");
				}
				break;	
		...
		private CheckBox ck_again;
		private EditText edit_password;
		private CareDialog creatCareDialog(){
			CareDialog.Builder builder=new CareDialog.Builder(mContext);
			builder.setTitle(mContext.getString(com.cappu.internal.R.string.dataconn_title));
	
			LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
			final View layout = inflater.inflate(com.cappu.internal.R.layout.data_conn_secret_code, null);
			builder.setView(layout);
	
			ck_again = (CheckBox) layout.findViewById(com.cappu.internal.R.id.ck_showpassword);
			edit_password = (EditText) layout.findViewById(com.cappu.internal.R.id.edit_password);
			ck_again.setChecked(true);//zgz add default show
			if(ck_again.isChecked()){
				edit_password.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);
				}
			ck_again.setOnClickListener(new View.OnClickListener() {
			@Override
			...
			builder.setPositiveButton(mContext.getString(com.cappu.internal.R.string.dataconn_ok), new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {
					int isEnable = Settings.Global.getInt(mContext.getContentResolver(),"enable_data_conn_secretcode", 1);
					String pwd=Settings.Global.getString(mContext.getContentResolver(), "data_conn_secretcode");
					boolean isDataon=Settings.Global.getInt(mContext.getContentResolver(),Settings.Global.MOBILE_DATA, 1) == 1;
					boolean isVerify = (1==isEnable) && edit_password.getText().toString().equals(pwd);
					...
					if(isVerify){
						...
						mContext.sendBroadcast(intent);
						Toast.makeText(mContext, mContext.getString(com.cappu.internal.R.string.dataconn_password_error), Toast.LENGTH_LONG).show();
					}
					
					edit_password.setText("");
				}
			});
			builder.setNegativeButton(mContext.getString(com.cappu.internal.R.string.dataconn_cancel), new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {
					// add by y.haiyang (start)
					Intent intent = new Intent("android.net.conn.CONNECTIVITY_CANCEL");
					mContext.sendBroadcast(intent);
			...
			CareDialog careDialog=builder.create();
			careDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_SYSTEM_ALERT);
			careDialog.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_HIDDEN|WindowManager.LayoutParams.SOFT_INPUT_ADJUST_UNSPECIFIED);
			careDialog.getWindow().setGravity(Gravity.TOP);
			WindowManager.LayoutParams lp =careDialog.getWindow().getAttributes();
            		lp.y=80; 
			return careDialog;
	}
	//zhaogangzhu@20151013  dataconn secretcode end @}
    ...
    private class ConnectivityServiceReceiver extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            ...
            synchronized (mSynchronizedObject) {
				...
				//zhaogangzhu@20151013  add
				else if ("android.net.conn.CAREDIALOGSHOW".equals(action)) {

					if (mCareDialog == null) {
						mCareDialog = creatCareDialog();
						mCareDialog.show();
					} else {
    ...
    /**
     * M: for design change, mms can be sent while mobile data is OFF
     * handle capability and default route
     */
    private void rematchCapabilityClearDefault(NetworkAgentInfo newNetwork,
            NetworkCapabilities networkCapabilities) {
        if (DBG) log("rematchCapabilityClearDefault");
        for (int i = 0; i < newNetwork.networkRequests.size(); i++) {
            NetworkRequest nr = newNetwork.networkRequests.valueAt(i);
            NetworkRequestInfo nri = mNetworkRequests.get(nr);

            if (DBG) log("checking requestId:" + nri.request.requestId);

            boolean satisfied = nri.request.networkCapabilities.satisfiedByNetworkCapabilities(
              newNetwork.networkCapabilities);
            if (DBG) log("satisfied:" + satisfied);
            if (!satisfied) {
                if (mDefaultRequest.requestId == nri.request.requestId) {
                    try {
                    ...
                    if (DBG) log("clear default network");
                    mNetworkForRequestId.remove(mDefaultRequest.requestId);
                    mLegacyTypeTracker.remove(newNetwork.networkInfo.getType(), newNetwork, true);
                    newNetwork.networkRequests.remove(nri.request.requestId);
    ...
    /**
     * M version no way to know if the MMS used network is created

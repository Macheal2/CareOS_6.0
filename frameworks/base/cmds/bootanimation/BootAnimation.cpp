...
bool BootAnimation::threadLoop()
{
    ...
    if (resourcePath != NULL) {
       ...
       //dengying bootaudio
       char pSelection[PROPERTY_VALUE_MAX] = {"0"};
       property_get("persist.sys.bootaudio", pSelection, "");

	if(strcmp(pSelection, "1") == 0){
		bPlayMP3 = true;
	}else if(strcmp(pSelection, "0")== 0){
		bPlayMP3 = false;
	} else {
        	bPlayMP3 = true;
        }
        ALOGD("dengyingBootAnimation: threadLoop bPlayMP3  = %d,pSelection=%s", bPlayMP3,pSelection);
        //end

	if(bPlayMP3){
            mediastatus = mediaplayer->start();
			ALOGD("[dengyingBootAnimation] threadLoop mediaplayer->start()");
         }
...
bool BootAnimation::ETC1movie()
{
    ...
    nsecs_t lastFrame = systemTime();
    nsecs_t frameDuration = s2ns(1) / animation.fps;
    ALOGD("[BootAnimation %s %d]lastFrame=%lld,frameDuration=%lld,ms=%lld",__FUNCTION__,__LINE__,lastFrame,frameDuration,ns2ms(systemTime()));

    for (size_t i=0 ; i<pcount ; i++) {
        const Animation::Part& part(animation.parts[i]);
        const size_t fcount = part.frames.size();
        ALOGD("[BootAnimation %s %d]i=%d,i<pcount=%d,r<part.count=%d,j<fcount=%d",__FUNCTION__,__LINE__,i,pcount,part.count,fcount);
        glBindTexture(GL_TEXTURE_2D, 0);
        ...
        if (part.count != 1) {
            for (size_t j=0 ; j<fcount ; j++) {
                const Animation::Frame& frame(part.frames[j]);
                glDeleteTextures(1, &frame.tid);
                ALOGD("[BootAnimation %s %d]del,part.count=%d,j=%d,fcount=%d",__FUNCTION__,__LINE__,part.count,j,fcount);
            }
        }
    }
